...
- Jobs persist under `~/.alphaclaw/cron/` so restarts don’t lose schedules.
...
alphaclaw cron list
alphaclaw cron run <job-id> --force
alphaclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.alphaclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `alphaclaw cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, AlphaClaw defaults to `announce`.
...
- Job store: `~/.alphaclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.alphaclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.alphaclaw/cron/jobs.json",
...
alphaclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
alphaclaw cron edit <jobId> --agent ops
alphaclaw cron edit <jobId> --clear-agent
```
...
```bash
alphaclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`alphaclaw system event`](/cli/system).
