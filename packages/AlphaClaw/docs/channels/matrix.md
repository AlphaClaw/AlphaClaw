...
Matrix is an open, decentralized messaging protocol. AlphaClaw connects as a Matrix **user**
...
```bash
alphaclaw plugins install @alphaclaw/matrix
```

Local checkout (when running from a git repo):

```bash
alphaclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `alphaclaw plugins install @alphaclaw/matrix`
   - From a local checkout: `alphaclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: AlphaClaw calls the same
     login endpoint, stores the access token in `~/.alphaclaw/credentials/matrix/credentials.json`,
...
- On first connection, AlphaClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  AlphaClaw logs a warning.
...
`~/.alphaclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `alphaclaw pairing list matrix`
  - `alphaclaw pairing approve matrix <CODE>`
...
- On startup, AlphaClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
