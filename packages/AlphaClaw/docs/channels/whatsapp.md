...
2. Configure WhatsApp in `~/.alphaclaw/alphaclaw.json`.
3. Run `alphaclaw channels login` to scan the QR code (Linked Devices).
4. Start the gateway.
...
WhatsApp requires a real mobile number for verification. VoIP and virtual numbers are usually blocked. There are two supported ways to run AlphaClaw on WhatsApp:

### Dedicated number (recommended)

Use a **separate phone number** for AlphaClaw. Best UX, clean routing, no self-chat quirks. Ideal setup: **spare/old Android phone + eSIM**. Leave it on Wi‑Fi and power, and link it via QR.

**WhatsApp Business:** You can use WhatsApp Business on the same device with a different number. Great for keeping your personal WhatsApp separate — install WhatsApp Business and register the AlphaClaw number there.
...
Quick fallback: run AlphaClaw on **your own number**. Message yourself (WhatsApp “Message yourself”) for testing so you don’t spam contacts. Expect to read verification codes on your main phone during setup and experiments. **Must enable self-chat mode.**
...
Self-chat replies default to `[{identity.name}]` when set (otherwise `[alphaclaw]`)
...
- Early AlphaClaw builds supported Twilio’s WhatsApp Business integration.
...
- Login command: `alphaclaw channels login` (QR via Linked Devices).
- Multi-account login: `alphaclaw channels login --account <id>` (`<id>` = `accountId`).
- Default account (when `--account` is omitted): `default` if present, otherwise the first configured account id (sorted).
- Credentials stored in `~/.alphaclaw/credentials/whatsapp/<accountId>/creds.json`.
- Backup copy at `creds.json.bak` (restored on corruption).
- Legacy compatibility: older installs stored Baileys files directly in `~/.alphaclaw/credentials/`.
- Logout: `alphaclaw channels logout` (or `--account <id>`) deletes WhatsApp auth state (but keeps shared `oauth.json`).
...
- **DM policy**: `channels.whatsapp.dmPolicy` controls direct chat access (default: `pairing`).
  - Pairing: unknown senders get a pairing code (approve via `alphaclaw pairing approve whatsapp <code>`; codes expire after 1 hour).
...
If you run AlphaClaw on your **personal WhatsApp number**, enable `channels.whatsapp.selfChatMode` (see sample above).
...
**Will AlphaClaw message random contacts when I link WhatsApp?**  
No. Default DM policy is **pairing**, so unknown senders only get a pairing code and their message is **not processed**. AlphaClaw only replies to chats it receives, or to sends you explicitly trigger (agent/CLI).

**How does pairing work on WhatsApp?**  
Pairing is a DM gate for unknown senders:

- First DM from a new sender returns a short code (message is not processed).
- Approve with: `alphaclaw pairing approve whatsapp <code>` (list with `alphaclaw pairing list whatsapp`).
- Codes expire after 1 hour; pending requests are capped at 3 per channel.

**Can multiple people use different AlphaClaw instances on one WhatsApp number?**  
...
- Media:
  ...
  - Animated GIFs: WhatsApp expects MP4 with `gifPlayback: true` for inline looping.
    - CLI: `alphaclaw message send --media <mp4> --gif-playback`
...
- Best results: OGG/Opus. AlphaClaw rewrites `audio/ogg` to `audio/ogg; codecs=opus`.
...
- Log file: `/tmp/alphaclaw/alphaclaw-YYYY-MM-DD.log` (configurable).
...
- Fix: run `alphaclaw channels login` on the gateway host and scan the QR (WhatsApp → Settings → Linked Devices).

**Linked but disconnected / reconnect loop**

- Symptom: `channels status` shows `running, disconnected` or warns “Linked but disconnected”.
- Fix: `alphaclaw doctor` (or restart the gateway). If it persists, relink via `channels login` and inspect `alphaclaw logs --follow`.
