...
read_when:
  - You want to understand what “context” means in AlphaClaw
...
“Context” is **everything AlphaClaw sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (AlphaClaw-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How AlphaClaw builds the system prompt

The system prompt is **AlphaClaw-owned** and rebuilt each run. It includes:
...
By default, AlphaClaw injects a fixed set of workspace files (if present):
