...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in AlphaClaw
...
Running AlphaClaw via Node with `tsx` fails at startup with:

```
[alphaclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `alphaclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch alphaclaw.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node alphaclaw.mjs status` works on Node 25.
