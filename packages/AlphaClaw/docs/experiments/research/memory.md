...
read_when:
  - Designing workspace memory (~/.alphaclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep AlphaClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.alphaclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.alphaclaw/workspace` as canonical human-readable memory.
...
```
~/.alphaclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in AlphaClaw**, but keep a separable core library.

### Why integrate into AlphaClaw?

- AlphaClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `alphaclaw memory recall "…" --k 25 --since 30d`
  - `alphaclaw memory reflect --since 7d`
...
Pragmatic take for `~/.alphaclaw/workspace`:
