---
summary: "All configuration options for ~/.alphaclaw/alphaclaw.json with examples"
read_when:
...
AlphaClaw reads an optional **JSON5** config from `~/.alphaclaw/alphaclaw.json` (comments + trailing commas allowed).

If the file is missing, AlphaClaw uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.alphaclaw/workspace`). You usually only need a config to:
...
AlphaClaw only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `alphaclaw doctor`, `alphaclaw logs`, `alphaclaw health`, `alphaclaw status`, `alphaclaw service`, `alphaclaw help`).
- Run `alphaclaw doctor` to see the exact issues.
- Run `alphaclaw doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `alphaclaw config set`. Keep a backup of `~/.alphaclaw/alphaclaw.json`.
...
```bash
alphaclaw gateway call config.get --params '{}' # capture payload.hash
alphaclaw gateway call config.apply --params '{
...
```bash
alphaclaw gateway call config.get --params '{}' # capture payload.hash
alphaclaw gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.alphaclaw/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@alphaclaw", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.alphaclaw/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.alphaclaw/workspace" }],
}
```
...
{ "$include": "/etc/alphaclaw/base.json5" } // absolute
...
```json5
// ~/.alphaclaw/clients/mueller/agents.json5
[
...
```json5
// ~/.alphaclaw/clients/mueller/broadcast.json5
{
...
AlphaClaw reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, AlphaClaw runs your login shell and imports only the missing expected keys (never overrides).
...
AlphaClaw stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.alphaclaw/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.alphaclaw/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`)
...
- `~/.alphaclaw/agent/*` (migrated by `alphaclaw doctor` into `~/.alphaclaw/agents/<defaultAgentId>/agent/*`)
...
On first use, AlphaClaw imports `oauth.json` entries into `auth-profiles.json`.
...
If set, AlphaClaw derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/alphaclaw/alphaclaw-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/alphaclaw/alphaclaw.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/alphaclaw/alphaclaw.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.alphaclaw/credentials/whatsapp/biz
          // authDir: "~/.alphaclaw/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `alphaclaw doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@alphaclaw", "alphaclaw"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@alphaclaw"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.alphaclaw/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.alphaclaw/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.alphaclaw/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.alphaclaw/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.alphaclaw/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.alphaclaw/workspace-home" },
      { id: "work", workspace: "~/.alphaclaw/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `alphaclaw.json`).
...
AlphaClaw starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["alphaclaw-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-alphaclaw",
...
AlphaClaw starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `alphaclaw plugins install @alphaclaw/mattermost` (or `./extensions/mattermost` from a git checkout).
...
AlphaClaw starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
AlphaClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, AlphaClaw
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/alphaclaw" } },
}
```
...
When a file exceeds this limit, AlphaClaw logs a warning and injects a truncated
...
message envelopes). If unset, AlphaClaw uses the host timezone at runtime.
...
`[alphaclaw]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[alphaclaw]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[alphaclaw]"`, AlphaClaw will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, AlphaClaw generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.alphaclaw/settings/tts.json",
      elevenlabs: {
...
AlphaClaw also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, AlphaClaw currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `alphaclaw configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:alphaclaw`: all built-in AlphaClaw tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.alphaclaw/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.alphaclaw/sandboxes",
        docker: {
          image: "alphaclaw-sandbox:bookworm-slim",
          containerPrefix: "alphaclaw-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "alphaclaw-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "alphaclaw-sandbox-browser:bookworm-slim",
          containerPrefix: "alphaclaw-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
AlphaClaw uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.alphaclaw/agents/<agentId>/agent/models.json` or by defining the same schema inside your
AlphaClaw config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, AlphaClaw writes/merges a `models.json` into
`~/.alphaclaw/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. AlphaClaw uses
...
- Shortcut: `alphaclaw onboard --auth-choice opencode-zen`.
...
Shortcut: `alphaclaw onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `alphaclaw onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `alphaclaw onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `alphaclaw onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `alphaclaw onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `alphaclaw onboard --auth-choice minimax-api`.
...
- Override the agent config root with `OPENCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.alphaclaw/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.alphaclaw/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.alphaclaw/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, AlphaClaw stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.alphaclaw/skills` (workspace skills
...
from `~/.alphaclaw/extensions`, `<workspace>/.alphaclaw/extensions`, plus any
...
### `browser` (alphaclaw-managed browser)

AlphaClaw can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for alphaclaw and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (AlphaClaw.app menubar, or `alphaclaw gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      alphaclaw: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/alphaclaw" }
...
- Examples: `"/ui"`, `"/alphaclaw"`, `"/apps/alphaclaw"`.
...
- When a connection comes from one of these IPs, AlphaClaw uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `alphaclaw gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
...
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  ...
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. AlphaClaw
...
- AlphaClaw.app watches `~/.alphaclaw/alphaclaw.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.alphaclaw/alphaclaw.json` (or `OPENCLAW_CONFIG_PATH`) and applies changes automatically.
...
- `~/.alphaclaw/alphaclaw.json` (or `OPENCLAW_CONFIG_PATH`)
...
- `alphaclaw --dev …` → uses `~/.alphaclaw-dev` + shifts ports from base `19001`
- `alphaclaw --profile <name> …` → uses `~/.alphaclaw-<name>` (port via config/env/flags)
...
```bash
OPENCLAW_CONFIG_PATH=~/.alphaclaw/a.json \
OPENCLAW_STATE_DIR=~/.alphaclaw-a \
alphaclaw gateway --port 19001
```
...
Gmail helper config (used by `alphaclaw webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "alphaclaw@gmail.com",
...
Note: when `tailscale.mode` is on, AlphaClaw defaults `serve.path` to `/` so
...
Default root: `~/.alphaclaw/workspace/canvas`  
Default port: `18793` (chosen to avoid the alphaclaw browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__alphaclaw__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__alphaclaw__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.alphaclaw/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.alphaclaw/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.alphaclaw/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.alphaclaw/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_alphaclaw-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `alphaclaw` (advertises `alphaclaw.local`). Override with `OPENCLAW_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_alphaclaw-gw._tcp` under `~/.alphaclaw/dns/` using the configured discovery domain (example: `alphaclaw.internal.`).
