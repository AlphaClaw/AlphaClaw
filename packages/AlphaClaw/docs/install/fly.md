---
title: Fly.io
description: Deploy AlphaClaw on Fly.io
---

# Fly.io Deployment

**Goal:** AlphaClaw Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
fly apps create my-alphaclaw

# Create a persistent volume (1GB is usually enough)
fly volumes create alphaclaw_data --size 1 --region iad
```
...
```toml
app = "my-alphaclaw"  # Your app name
primary_region = "iad"
...
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `alphaclaw.json` where they could be accidentally exposed or logged.
...
**Note:** With `OPENCLAW_STATE_DIR=/data`, the config path is `/data/alphaclaw.json`.
...
Or visit `https://my-alphaclaw.fly.dev/`
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/alphaclaw.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/alphaclaw.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/alphaclaw.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/alphaclaw.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/alphaclaw.json"
```
...
```bash
# List current IPs
fly ips list -a my-alphaclaw

# Release public IPs
fly ips release <public-ipv4> -a my-alphaclaw
fly ips release <public-ipv6> -a my-alphaclaw
...
fly ips allocate-v6 --private -a my-alphaclaw
```
...
fly proxy 3000:3000 -a my-alphaclaw
...
```bash
fly ssh console -a my-alphaclaw
```
