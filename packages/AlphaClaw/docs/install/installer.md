...
read_when:
  - You want to understand `alphaclaw.ai/install.sh`
...
AlphaClaw ships two installer scripts (served from `alphaclaw.ai`):

- `https://alphaclaw.ai/install.sh` — “recommended” installer (global npm install by default; can also install from a GitHub checkout)
- `https://alphaclaw.ai/install-cli.sh` — non-root-friendly CLI installer (installs into a prefix with its own Node)
- `https://alphaclaw.ai/install.ps1` — Windows PowerShell installer (npm by default; optional git install)

To see the current flags/behavior, run:

```bash
curl -fsSL https://alphaclaw.ai/install.sh | bash -s -- --help
```

Windows (PowerShell) help:

```powershell
& ([scriptblock]::Create((iwr -useb https://alphaclaw.ai/install.ps1))) -?
```

If the installer completes but `alphaclaw` is not found in a new terminal, it’s usually a Node/npm PATH issue. See: [Install](/install#nodejs--npm-path-sanity).
...
- Choose install method:
  - `npm` (default): `npm install -g alphaclaw@latest`
...
- If upgrading an existing install: runs `alphaclaw doctor --non-interactive` (best effort).
- For git installs: runs `alphaclaw doctor --non-interactive` after install/update (best effort).
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL https://alphaclaw.ai/install.sh | bash
```

### Discoverability / “git install” prompt

If you run the installer while **already inside a AlphaClaw source checkout** (detected via `package.json` + `pnpm-workspace.yaml`), it prompts:
...
This script installs `alphaclaw` into a prefix (default: `~/.alphaclaw`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you don’t want to touch the system Node/npm.

Help:

```bash
curl -fsSL https://alphaclaw.ai/install-cli.sh | bash -s -- --help
```
...
- Choose install method:
  - `npm` (default): `npm install -g alphaclaw@latest`
  - `git`: clone/build a source checkout and install a wrapper script
- Runs `alphaclaw doctor --non-interactive` on upgrades and git installs (best effort).

Examples:

```powershell
iwr -useb https://alphaclaw.ai/install.ps1 | iex
```

```powershell
iwr -useb https://alphaclaw.ai/install.ps1 | iex -InstallMethod git
```

```powershell
iwr -useb https://alphaclaw.ai/install.ps1 | iex -InstallMethod git -GitDir "C:\\alphaclaw"
```
...
- **"alphaclaw" is not recognized**: your npm global bin folder is not on PATH. Most systems use
