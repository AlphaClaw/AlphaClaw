...
This guide summarizes a sane workflow for working on the pi integration in AlphaClaw.
...
- Trigger the agent directly:
  - `pnpm alphaclaw agent --message "Hello" --thinking low`
...
State lives under the AlphaClaw state directory. Default is `~/.alphaclaw`. If `OPENCLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `alphaclaw.json` for config
...
- https://docs.alphaclaw.ai/testing
- https://docs.alphaclaw.ai/start/getting-started
