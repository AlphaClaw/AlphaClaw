---
summary: "AlphaClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up AlphaClaw on a Raspberry Pi
  - Running AlphaClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# AlphaClaw on Raspberry Pi

## Goal

Run a persistent, always-on AlphaClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most AlphaClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled alphaclaw

# Enable if not
sudo systemctl enable alphaclaw

# Start on boot
sudo systemctl start alphaclaw
```
...
```bash
# Check logs
journalctl -u alphaclaw --no-pager -n 100

# Common fix: rebuild
cd ~/alphaclaw  # if using hackable install
npm run build
sudo systemctl restart alphaclaw
```
