---
summary: "Use Venice AI privacy-focused models in AlphaClaw"
read_when:
  - You want privacy-focused inference in AlphaClaw
...
```bash
alphaclaw onboard --auth-choice venice-api-key
```
...
```bash
alphaclaw chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, AlphaClaw shows all available Venice models. Pick based on your needs:
...
```bash
alphaclaw models set venice/claude-opus-45
alphaclaw models set venice/llama-3.3-70b
```

List all available models:

```bash
alphaclaw models list | grep venice
```

## Configure via `alphaclaw configure`

1. Run `alphaclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
AlphaClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
alphaclaw chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
alphaclaw chat --model venice/claude-opus-45

# Use uncensored model
alphaclaw chat --model venice/venice-uncensored

# Use vision model with image
alphaclaw chat --model venice/qwen3-vl-235b-a22b

# Use coding model
alphaclaw chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `alphaclaw models list` to see currently available models. Some models may be temporarily offline.
