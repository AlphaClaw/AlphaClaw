...
- Publish as `alphaclaw/plugin-sdk` (or export from core under `alphaclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `AlphaClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: AlphaClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: AlphaClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: AlphaClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: AlphaClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: AlphaClawConfig): string;
  };
};
```
...
- Introduce `alphaclaw/plugin-sdk`.
- Add `api.runtime` to `AlphaClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `alphaclawRuntime: ">=2026.2.0"`).
