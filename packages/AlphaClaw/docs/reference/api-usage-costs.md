...
AlphaClaw features that can generate provider usage or paid API calls.
...
- `alphaclaw status --usage` and `alphaclaw channels list` show provider **usage windows**
...
AlphaClaw can pick up credentials from:
...
- `alphaclaw status --usage`
- `alphaclaw models status --json`
...
`alphaclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
