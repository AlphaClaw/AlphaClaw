...
This document explains how AlphaClaw manages sessions end-to-end:
...
AlphaClaw is designed around a single **Gateway process** that owns session state.
...
AlphaClaw persists sessions in two layers:
...
- Transcripts: `~/.alphaclaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

AlphaClaw resolves these via `src/config/sessions.ts`.
...
AlphaClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (AlphaClaw consumes the events, but Pi decides when to compact).
...
AlphaClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, AlphaClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, AlphaClaw leaves it alone.
...
- `alphaclaw status` (CLI)
- `alphaclaw sessions` / `sessions --json`
...
AlphaClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- AlphaClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, AlphaClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
AlphaClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but AlphaClaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `alphaclaw status`.
