...
read_when:
  - You need detailed behavior for alphaclaw onboard
...
This page is the full reference for `alphaclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.alphaclaw/alphaclaw.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `alphaclaw doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `alphaclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `alphaclaw health`.
    - `alphaclaw status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.alphaclaw/.env` so launchd can read it.
...
- OAuth credentials: `~/.alphaclaw/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.alphaclaw/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`alphaclaw onboard`](/cli/onboard)
