...
- **gateway host** → `alphaclaw` process on the gateway machine
...
`~/.alphaclaw/exec-approvals.json` directly.

CLI: `alphaclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
