---
summary: "Agent tool surface for AlphaClaw (browser, canvas, nodes, message, cron) replacing legacy `alphaclaw-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `alphaclaw-*` skills
title: "Tools"
---

# Tools (AlphaClaw)

AlphaClaw exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `alphaclaw-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `alphaclaw.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, AlphaClaw logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`alphaclaw node run`).
...
- Requires a Brave API key (recommended: `alphaclaw configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated AlphaClaw-managed browser.
...
- Quick smoke: `alphaclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `alphaclaw gateway` restart in-place)
