...
AlphaClaw can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere AlphaClaw can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, AlphaClaw defaults
...
TTS config lives under `messages.tts` in `alphaclaw.json`.
...
- If `provider` is **unset**, AlphaClaw prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.alphaclaw/settings/tts.json`, override with `OPENCLAW_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, AlphaClaw retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, AlphaClaw:
...
Discord note: `/tts` is a built-in Discord command, so AlphaClaw registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from AlphaClaw
```
