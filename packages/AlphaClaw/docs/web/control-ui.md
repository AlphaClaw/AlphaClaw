...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/alphaclaw`)
...
If the page fails to load, start the Gateway first: `alphaclaw gateway`.
...
you revoke it with `alphaclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.alphaclaw/alphaclaw.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. AlphaClaw
...
AlphaClaw **blocks** Control UI connections without device identity.
