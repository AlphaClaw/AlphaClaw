...
如果您刚安装完 AlphaClaw，可以直接运行向导，根据提示添加飞书：
...
- `alphaclaw gateway status` - 查看网关运行状态
- `alphaclaw logs --follow` - 查看实时日志
...
- `alphaclaw gateway status` - 查看网关运行状态
- `alphaclaw gateway restart` - 重启网关以应用新配置
- `alphaclaw logs --follow` - 查看实时日志
...
1. 运行 `alphaclaw channels add` 添加了 Feishu 渠道
2. 网关处于启动状态（可通过 `alphaclaw gateway status` 检查状态）
...
```bash
alphaclaw pairing approve feishu <配对码>
```
...
- **批准配对**：
  ```bash
  alphaclaw pairing list feishu      # 查看待审批列表
  alphaclaw pairing approve feishu <CODE>  # 批准
  ```
...
1. 启动网关并在群组中 @机器人发消息
2. 运行 `alphaclaw logs --follow` 查看日志中的 `chat_id`
...
1. 启动网关并给机器人发消息
2. 运行 `alphaclaw logs --follow` 查看日志中的 `open_id`
...
| -------------------------- | ----------------- |
| `alphaclaw gateway status`  | 查看网关运行状态  |
| `alphaclaw gateway install` | 安装/启动网关服务 |
| `alphaclaw gateway stop`    | 停止网关服务      |
| `alphaclaw gateway restart` | 重启网关服务      |
| `alphaclaw logs --follow`   | 实时查看日志输出  |
...
3. 检查是否选择了 **长连接** 模式
4. 检查应用权限是否完整
5. 检查网关是否正在运行：`alphaclaw gateway status`
6. 查看实时日志：`alphaclaw logs --follow`
