...
你可以在 `~/.alphaclaw/alphaclaw.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/alphaclaw.log"
  }
}
```
...
```bash
alphaclaw channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.alphaclaw/alphaclaw.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/alphaclaw/alphaclaw-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- AlphaClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `alphaclaw plugins enable diagnostics-otel` 启用插件。
...
- `alphaclaw.tokens`（计数器，属性：`alphaclaw.token`、`alphaclaw.channel`、`alphaclaw.provider`、`alphaclaw.model`）
- `alphaclaw.cost.usd`（计数器，属性：`alphaclaw.channel`、`alphaclaw.provider`、`alphaclaw.model`）
- `alphaclaw.run.duration_ms`（直方图，属性：`alphaclaw.channel`、`alphaclaw.provider`、`alphaclaw.model`）
- `alphaclaw.context.tokens`（直方图，属性：`alphaclaw.context`、`alphaclaw.channel`、`alphaclaw.provider`、`alphaclaw.model`）

消息流：

- `alphaclaw.webhook.received`（计数器，属性：`alphaclaw.channel`、`alphaclaw.webhook`）
- `alphaclaw.webhook.error`（计数器，属性：`alphaclaw.channel`、`alphaclaw.webhook`）
- `alphaclaw.webhook.duration_ms`（直方图，属性：`alphaclaw.channel`、`alphaclaw.webhook`）
- `alphaclaw.message.queued`（计数器，属性：`alphaclaw.channel`、`alphaclaw.source`）
- `alphaclaw.message.processed`（计数器，属性：`alphaclaw.channel`、`alphaclaw.outcome`）
- `alphaclaw.message.duration_ms`（直方图，属性：`alphaclaw.channel`、`alphaclaw.outcome`）

队列 + 会话：

- `alphaclaw.queue.lane.enqueue`（计数器，属性：`alphaclaw.lane`）
- `alphaclaw.queue.lane.dequeue`（计数器，属性：`alphaclaw.lane`）
- `alphaclaw.queue.depth`（直方图，属性：`alphaclaw.lane` 或 `alphaclaw.channel=heartbeat`）
- `alphaclaw.queue.wait_ms`（直方图，属性：`alphaclaw.lane`）
- `alphaclaw.session.state`（计数器，属性：`alphaclaw.state`、`alphaclaw.reason`）
- `alphaclaw.session.stuck`（计数器，属性：`alphaclaw.state`）
- `alphaclaw.session.stuck_age_ms`（直方图，属性：`alphaclaw.state`）
- `alphaclaw.run.attempt`（计数器，属性：`alphaclaw.attempt`）

### 导出的 span（名称 + 关键属性）

- `alphaclaw.model.usage`
  - `alphaclaw.channel`、`alphaclaw.provider`、`alphaclaw.model`
  - `alphaclaw.sessionKey`、`alphaclaw.sessionId`
  - `alphaclaw.tokens.*`（input/output/cache_read/cache_write/total）
- `alphaclaw.webhook.processed`
  - `alphaclaw.channel`、`alphaclaw.webhook`、`alphaclaw.chatId`
- `alphaclaw.webhook.error`
  - `alphaclaw.channel`、`alphaclaw.webhook`、`alphaclaw.chatId`、`alphaclaw.error`
- `alphaclaw.message.processed`
  - `alphaclaw.channel`、`alphaclaw.outcome`、`alphaclaw.chatId`、`alphaclaw.messageId`、`alphaclaw.sessionKey`、`alphaclaw.sessionId`、`alphaclaw.reason`
- `alphaclaw.session.stuck`
  - `alphaclaw.state`、`alphaclaw.ageMs`、`alphaclaw.queueDepth`、`alphaclaw.sessionKey`、`alphaclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `alphaclaw doctor`。
