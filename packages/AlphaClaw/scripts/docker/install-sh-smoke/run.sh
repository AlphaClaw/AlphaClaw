...
INSTALL_URL="${ALPHACLAW_INSTALL_URL:-https://alphaclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${ALPHACLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ALPHACLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="alphaclaw"
PACKAGE_NAME="${ALPHACLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ALPHACLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ALPHACLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${ALPHACLAW_INSTALL_LATEST_OUT:-}"
fi
