...
IMAGE_NAME="alphaclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="alphaclaw-net-e2e-$$"
GW_NAME="alphaclaw-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "ALPHACLAW_GATEWAY_TOKEN=$TOKEN" \
	  -e "ALPHACLAW_SKIP_CHANNELS=1" \
	  -e "ALPHACLAW_SKIP_GMAIL_WATCHER=1" \
	  -e "ALPHACLAW_SKIP_CRON=1" \
	  -e "ALPHACLAW_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
