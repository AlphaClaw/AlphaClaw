...
ZIP=${1:?"Usage: $0 AlphaClaw-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/alphaclaw/alphaclaw/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^AlphaClaw-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/alphaclaw/alphaclaw/releases/download/v${VERSION}/"}
