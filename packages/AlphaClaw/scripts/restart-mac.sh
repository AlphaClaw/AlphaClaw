#!/usr/bin/env bash
# Reset AlphaClaw like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${ALPHACLAW_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="AlphaClaw.app/Contents/MacOS/AlphaClaw"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/AlphaClaw"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/AlphaClaw"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/AlphaClaw"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.alphaclaw.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/alphaclaw-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${ALPHACLAW_RESTART_LOG:-/tmp/alphaclaw-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${ALPHACLAW_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.alphaclaw/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  ALPHACLAW_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node alphaclaw.mjs daemon install --force --runtime node"
      log "  node alphaclaw.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.alphaclaw/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.alphaclaw.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing AlphaClaw instances"
kill_all_alphaclaw
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product AlphaClaw"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/AlphaClaw.app" ]]; then
    APP_BUNDLE="/Applications/AlphaClaw.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/AlphaClaw.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/AlphaClaw.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/AlphaClaw.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set ALPHACLAW_APP_BUNDLE to your installed AlphaClaw.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node alphaclaw.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node alphaclaw.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".alphaclaw", "alphaclaw.json"), "utf8");
