...
const shouldBuild = () => {
  if (env.ALPHACLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message) => {
  if (env.ALPHACLAW_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[alphaclaw] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["alphaclaw.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
