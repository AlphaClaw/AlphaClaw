...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/alphaclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "AlphaClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/alphaclaw/scripts/mobile-reauth.sh'" --id alphaclaw-auth
        ;;
    *)
