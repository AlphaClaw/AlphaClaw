 ...
 * `alphaclaw update`. Use it to verify the completion installation flow
...
\`alphaclaw update\`, \`alphaclaw doctor\`, and \`alphaclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# AlphaClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`alphaclaw completion --install\` later to enable.`));
    return;
  }
