---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for AlphaClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, AlphaClaw cron scheduling for periodic checks, or version status checks on a machine running AlphaClaw (laptop, workstation, Pi, VPS).
---

# AlphaClaw Host Hardening

## Overview

Assess and harden the host running AlphaClaw, then align it to a user-defined risk tolerance without breaking access. Use AlphaClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim AlphaClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. AlphaClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if AlphaClaw can access sensitive data.
...
### 2) Run AlphaClaw security audits (read-only)

As part of the default read-only checks, run `alphaclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `alphaclaw security audit` (faster, non-probing)
2. `alphaclaw security audit --json` (structured output)

Offer to apply AlphaClaw safe defaults (numbered):

1. `alphaclaw security audit --fix`

Be explicit that `--fix` only tightens AlphaClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check AlphaClaw version/update status (read-only)

As part of the default read-only checks, run `alphaclaw update status`.
...
- Credential hygiene notes (location of AlphaClaw creds, prefer disk encryption)
...
- AlphaClaw security audit (re-run)
...
After AlphaClaw install or first hardening pass, run at least one baseline audit and version check:

- `alphaclaw security audit`
- `alphaclaw security audit --deep`
- `alphaclaw update status`

Ongoing monitoring is recommended. Use the AlphaClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `alphaclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `alphaclaw update status`
...
Before creating, `alphaclaw cron list` and match on exact `name`. If found, `alphaclaw cron edit <id> ...`.
If not found, `alphaclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `alphaclaw update status` (preferred for source checkouts and channels)
2. `npm view alphaclaw version` (published npm version)

## AlphaClaw command accuracy

Use only supported commands and flags:

- `alphaclaw security audit [--deep] [--fix] [--json]`
- `alphaclaw status` / `alphaclaw status --deep`
- `alphaclaw health --json`
- `alphaclaw update status`
- `alphaclaw cron add|list|runs|run`

Do not invent CLI flags or imply AlphaClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by AlphaClaw compaction:
