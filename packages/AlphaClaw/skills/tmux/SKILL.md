...
metadata:
  { "alphaclaw": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---

# tmux Skill (AlphaClaw)
...
```bash
SOCKET_DIR="${ALPHACLAW_TMUX_SOCKET_DIR:-${CLAWDBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/alphaclaw-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/alphaclaw.sock"
SESSION=alphaclaw-python
...
- Use `ALPHACLAW_TMUX_SOCKET_DIR` (legacy `CLAWDBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$ALPHACLAW_TMUX_SOCKET_DIR/alphaclaw.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `ALPHACLAW_TMUX_SOCKET_DIR`).
