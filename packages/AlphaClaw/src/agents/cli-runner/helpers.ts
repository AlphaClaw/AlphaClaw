...
/**
 * Cleanup suspended AlphaClaw CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: AlphaClawConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alphaclaw-cli-images-"));
  const paths: string[] = [];
