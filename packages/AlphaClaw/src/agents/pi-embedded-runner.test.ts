...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alphaclaw-embedded-agent-"));
...
const ensureModels = (cfg: AlphaClawConfig) => ensureAlphaClawModelsJson(cfg, agentDir) as unknown;
