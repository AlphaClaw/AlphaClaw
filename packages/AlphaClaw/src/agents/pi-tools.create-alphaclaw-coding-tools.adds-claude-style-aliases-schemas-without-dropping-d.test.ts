...
const defaultTools = createAlphaClawCodingTools();

describe("createAlphaClawCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alphaclaw-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createAlphaClawCodingTools();
    ...
    try {
      ...
      const contents = "Hello from alphaclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "alphaclaw-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "alphaclaw-workspace"),
    ...
    const tools = createAlphaClawCodingTools({ sandbox });
