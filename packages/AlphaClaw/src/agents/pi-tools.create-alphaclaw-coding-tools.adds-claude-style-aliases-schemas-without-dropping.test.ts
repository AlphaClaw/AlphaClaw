...
import { __testing, createAlphaClawCodingTools } from "./pi-tools.js";
...
describe("createAlphaClawCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createAlphaClawTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createAlphaClawCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alphaclaw-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "alphaclaw-outside.txt");
