...
function resolveExecConfig(cfg: AlphaClawConfig | undefined) {
...
export function createAlphaClawCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createAlphaClawReadTool(freshReadTool)];
    }
