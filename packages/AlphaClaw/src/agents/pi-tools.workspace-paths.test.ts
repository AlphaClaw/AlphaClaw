...
import { createAlphaClawCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("alphaclaw-ws-", async (workspaceDir) => {
      await withTempDir("alphaclaw-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createAlphaClawCodingTools({ workspaceDir });
    ...
    await withTempDir("alphaclaw-ws-", async (workspaceDir) => {
      await withTempDir("alphaclaw-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello alphaclaw");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("alphaclaw-ws-", async (workspaceDir) => {
      await withTempDir("alphaclaw-override-", async (overrideDir) => {
        const tools = createAlphaClawCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("alphaclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("alphaclaw-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "alphaclaw-sbx-test",
        ...
        const tools = createAlphaClawCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
