...
type WebSearchConfig = NonNullable<AlphaClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: AlphaClawConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("alphaclaw configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.alphaclaw.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://alphaclaw.ai",
      "X-Title": "AlphaClaw Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
