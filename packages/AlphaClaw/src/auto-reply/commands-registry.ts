...
import type { AlphaClawConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: AlphaClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AlphaClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as AlphaClawConfig),
...
export function getCommandDetection(_cfg?: AlphaClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AlphaClawConfig) {
