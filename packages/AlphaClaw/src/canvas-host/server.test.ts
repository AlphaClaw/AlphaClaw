...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("alphaclawCanvasA2UIAction");
    expect(out).toContain("alphaclawSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "alphaclaw-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("alphaclawSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.alphaclawA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__alphaclaw__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("alphaclaw-a2ui-host");
      expect(html).toContain("alphaclawCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__alphaclaw__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("alphaclawA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
