...
export function registerBrowserCli(program: Command) {
  const browser = program
    .command("browser")
    .description("Manage AlphaClaw's dedicated browser (Chrome/Chromium)")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples(
          ...
          "/cli/browser",
          "docs.alphaclaw.ai/cli/browser",
        )}\n`,
    )
    .action(() => {
      browser.outputHelp();
      defaultRuntime.error(
        danger(`Missing subcommand. Try: "${formatCliCommand("alphaclaw browser status")}"`),
      );
