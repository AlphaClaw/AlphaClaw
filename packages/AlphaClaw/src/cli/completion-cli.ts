...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# AlphaClaw Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(alphaclaw completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# AlphaClaw Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(alphaclaw completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $ALPHACLAW_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "alphaclaw") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c alphaclaw -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'alphaclaw gateway' and not 'alphaclaw other gateway', we need to check the sequence.
