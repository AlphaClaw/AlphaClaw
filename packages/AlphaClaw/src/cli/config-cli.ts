...
async function loadValidConfig() {
  ...
  defaultRuntime.error(`Run \`${formatCliCommand("alphaclaw doctor")}\` to repair, then retry.`);
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.alphaclaw.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
