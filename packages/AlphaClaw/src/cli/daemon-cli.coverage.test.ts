...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    ALPHACLAW_STATE_DIR: process.env.ALPHACLAW_STATE_DIR,
    ALPHACLAW_CONFIG_PATH: process.env.ALPHACLAW_CONFIG_PATH,
    ALPHACLAW_GATEWAY_PORT: process.env.ALPHACLAW_GATEWAY_PORT,
    ALPHACLAW_PROFILE: process.env.ALPHACLAW_PROFILE,
  };

  beforeEach(() => {
    process.env.ALPHACLAW_STATE_DIR = "/tmp/alphaclaw-cli-state";
    process.env.ALPHACLAW_CONFIG_PATH = "/tmp/alphaclaw-cli-state/alphaclaw.json";
    delete process.env.ALPHACLAW_GATEWAY_PORT;
    delete process.env.ALPHACLAW_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.ALPHACLAW_STATE_DIR !== undefined) {
      process.env.ALPHACLAW_STATE_DIR = originalEnv.ALPHACLAW_STATE_DIR;
    } else {
      delete process.env.ALPHACLAW_STATE_DIR;
    }

    if (originalEnv.ALPHACLAW_CONFIG_PATH !== undefined) {
      process.env.ALPHACLAW_CONFIG_PATH = originalEnv.ALPHACLAW_CONFIG_PATH;
    } else {
      delete process.env.ALPHACLAW_CONFIG_PATH;
    }

    if (originalEnv.ALPHACLAW_GATEWAY_PORT !== undefined) {
      process.env.ALPHACLAW_GATEWAY_PORT = originalEnv.ALPHACLAW_GATEWAY_PORT;
    } else {
      delete process.env.ALPHACLAW_GATEWAY_PORT;
    }

    if (originalEnv.ALPHACLAW_PROFILE !== undefined) {
      process.env.ALPHACLAW_PROFILE = originalEnv.ALPHACLAW_PROFILE;
    } else {
      delete process.env.ALPHACLAW_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ALPHACLAW_PROFILE: "dev",
        ALPHACLAW_STATE_DIR: "/tmp/alphaclaw-daemon-state",
        ALPHACLAW_CONFIG_PATH: "/tmp/alphaclaw-daemon-state/alphaclaw.json",
        ALPHACLAW_GATEWAY_PORT: "19001",
      },
