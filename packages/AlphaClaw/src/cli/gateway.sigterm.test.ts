...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "alphaclaw-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      ALPHACLAW_NO_RESPAWN: "1",
      ALPHACLAW_STATE_DIR: stateDir,
      ALPHACLAW_SKIP_CHANNELS: "1",
      ALPHACLAW_SKIP_GMAIL_WATCHER: "1",
      ALPHACLAW_SKIP_CRON: "1",
      ALPHACLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
      ALPHACLAW_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "alphaclaw-entry-bootstrap.mjs");
