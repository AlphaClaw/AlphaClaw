...
${formatHelpExamples([
  ['alphaclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['alphaclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'alphaclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'alphaclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['alphaclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'alphaclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.alphaclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.alphaclaw.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['alphaclaw agents set-identity --agent main --name "AlphaClaw" --emoji "🦞"', "Set name + emoji."],
  ["alphaclaw agents set-identity --agent main --avatar avatars/alphaclaw.png", "Set avatar path."],
  [
    "alphaclaw agents set-identity --workspace ~/.alphaclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "alphaclaw agents set-identity --identity-file ~/.alphaclaw/workspace/IDENTITY.md --agent main",
