...
const SEARCH_TOOL = "https://docs.alphaclaw.ai/mcp.SearchAlphaClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.alphaclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('alphaclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.alphaclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('alphaclaw docs "your query"')}`);
    }
    return;
  }
