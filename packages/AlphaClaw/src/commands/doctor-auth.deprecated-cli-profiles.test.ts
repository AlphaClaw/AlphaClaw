...
beforeEach(() => {
  originalAgentDir = process.env.ALPHACLAW_AGENT_DIR;
  originalPiAgentDir = process.env.PI_CODING_AGENT_DIR;
  tempAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "alphaclaw-auth-"));
...
afterEach(() => {
  if (originalAgentDir === undefined) {
    ...
    process.env.ALPHACLAW_AGENT_DIR = originalAgentDir;
  }
