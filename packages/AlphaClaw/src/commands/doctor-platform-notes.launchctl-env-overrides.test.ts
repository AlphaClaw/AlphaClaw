...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ALPHACLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("ALPHACLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ALPHACLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("ALPHACLAW_GATEWAY_PASSWORD");
  });
