...
function resolveMode(cfg: AlphaClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("AlphaClaw doctor");

  const root = await resolveAlphaClawPackageRoot({
  ...
  let cfg: AlphaClawConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("alphaclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("alphaclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("alphaclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("alphaclaw doctor --fix")}" to apply changes.`);
  }
