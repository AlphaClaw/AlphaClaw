...
beforeEach(() => {
  ...
  resolveAlphaClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ALPHACLAW_STATE_DIR;
  originalUpdateInProgress = process.env.ALPHACLAW_UPDATE_IN_PROGRESS;
  process.env.ALPHACLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "alphaclaw-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.ALPHACLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.ALPHACLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.ALPHACLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveAlphaClawPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadAlphaClawPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/alphaclaw.json",
    createConfigIO,
...
vi.mock("../infra/alphaclaw-root.js", () => ({
  resolveAlphaClawPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "alphaclaw-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.ALPHACLAW_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when ALPHACLAW_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/alphaclaw.json",
      exists: true,
      raw: "{}",
    ...
    process.env.ALPHACLAW_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
