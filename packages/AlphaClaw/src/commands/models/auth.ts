...
import { readConfigFileSnapshot, type AlphaClawConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: AlphaClawConfig, model: string): AlphaClawConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("alphaclaw plugins install")}\`.`,
    );
  }
