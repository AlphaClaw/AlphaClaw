...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAlphaClawAgentDir: vi.fn().mockReturnValue("/tmp/alphaclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/alphaclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/alphaclaw-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveAlphaClawAgentDir: mocks.resolveAlphaClawAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveAlphaClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/alphaclaw-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/alphaclaw-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/alphaclaw-agent-custom");
