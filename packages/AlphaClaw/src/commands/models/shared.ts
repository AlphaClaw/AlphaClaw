...
export async function updateConfig(
  mutator: (cfg: AlphaClawConfig) => AlphaClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: AlphaClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AlphaClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("alphaclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
