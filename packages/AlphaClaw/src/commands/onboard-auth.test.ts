...
const requireAgentDir = () => {
  const agentDir = process.env.ALPHACLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("ALPHACLAW_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.ALPHACLAW_OAUTH_DIR;
  });

  it("writes auth-profiles.json under ALPHACLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "alphaclaw-oauth-"));
    process.env.ALPHACLAW_STATE_DIR = tempStateDir;
    process.env.ALPHACLAW_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ALPHACLAW_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ALPHACLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "alphaclaw-minimax-"));
    process.env.ALPHACLAW_STATE_DIR = tempStateDir;
    process.env.ALPHACLAW_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ALPHACLAW_AGENT_DIR;
