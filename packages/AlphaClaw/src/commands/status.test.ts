...
beforeAll(() => {
  previousProfile = process.env.ALPHACLAW_PROFILE;
  process.env.ALPHACLAW_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.ALPHACLAW_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/alphaclaw-root.js", () => ({
  resolveAlphaClawPackageRoot: vi.fn().mockResolvedValue("/tmp/alphaclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/alphaclaw/pnpm-lock.yaml",
      markerPath: "/tmp/alphaclaw/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("AlphaClaw status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("alphaclaw status --all") ||
          l.includes("alphaclaw --profile isolated status --all") ||
          l.includes("alphaclaw status --all") ||
          l.includes("alphaclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.ALPHACLAW_GATEWAY_TOKEN = "abcd1234";
    try {
