...
const ETC_ALPHACLAW_DIR = path.join(ROOT_DIR, "etc", "alphaclaw");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "alphaclaw.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcAlphaClawPath(...parts: string[]) {
  return path.join(ETC_ALPHACLAW_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcAlphaClawPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "alphaclaw.json"))).toEqual({
      shared: true,
    });
  });
});
