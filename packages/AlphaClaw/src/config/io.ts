...
import type { AlphaClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareAlphaClawVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "ALPHACLAW_GATEWAY_TOKEN",
  "ALPHACLAW_GATEWAY_PASSWORD",
];
...
function coerceConfig(value: unknown): AlphaClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as AlphaClawConfig;
}
...
function stampConfigVersion(cfg: AlphaClawConfig): AlphaClawConfig {
...
function warnIfConfigFromFuture(cfg: AlphaClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareAlphaClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer AlphaClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: AlphaClawConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AlphaClawConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as AlphaClawConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as AlphaClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: AlphaClawConfig) {
    clearConfigCache();
...
// module scope. `ALPHACLAW_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ALPHACLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.ALPHACLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): AlphaClawConfig {
...
export async function writeConfigFile(cfg: AlphaClawConfig): Promise<void> {
  clearConfigCache();
