import type { AlphaClawConfig } from "./config.js";
...
function isTelegramConfigured(cfg: AlphaClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: AlphaClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: AlphaClawConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: AlphaClawConfig): boolean {
...
function isIMessageConfigured(cfg: AlphaClawConfig): boolean {
...
function isWhatsAppConfigured(cfg: AlphaClawConfig): boolean {
...
function isGenericChannelConfigured(cfg: AlphaClawConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: AlphaClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: AlphaClawConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: AlphaClawConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: AlphaClawConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: AlphaClawConfig, pluginId: string): AlphaClawConfig {
...
function enablePluginEntry(cfg: AlphaClawConfig, pluginId: string): AlphaClawConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
