...
import type { AlphaClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: AlphaClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: AlphaClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AlphaClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AlphaClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AlphaClawConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as AlphaClawConfig)),
    ),
  };
}
