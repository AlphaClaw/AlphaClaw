...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "alphaclaw-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".alphaclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<AlphaClawConfig> = {},
): AlphaClawConfig {
  const base: AlphaClawConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "alphaclaw"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".alphaclaw", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
