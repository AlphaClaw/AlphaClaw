...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ALPHACLAW_GATEWAY_PORT).toBe("18789");
    expect(env.ALPHACLAW_GATEWAY_TOKEN).toBe("secret");
    expect(env.ALPHACLAW_SERVICE_MARKER).toBe("alphaclaw");
    expect(env.ALPHACLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ALPHACLAW_SERVICE_VERSION).toBe("string");
    expect(env.ALPHACLAW_SYSTEMD_UNIT).toBe("alphaclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.ALPHACLAW_LAUNCHD_LABEL).toBe("ai.alphaclaw.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ALPHACLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ALPHACLAW_SYSTEMD_UNIT).toBe("alphaclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.ALPHACLAW_LAUNCHD_LABEL).toBe("ai.alphaclaw.work");
    }
  });
});
