...
export function resolveHooksConfig(cfg: AlphaClawConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-alphaclaw-token"] === "string"
      ? req.headers["x-alphaclaw-token"].trim()
      : "";
  if (headerToken) {
