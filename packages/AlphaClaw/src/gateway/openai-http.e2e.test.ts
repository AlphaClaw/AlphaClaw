    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "alphaclaw", messages: [{ role: "user", content: "hi" }] },
          { "x-alphaclaw-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-alphaclaw-agent-id": "beta",
            "x-alphaclaw-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
