...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.ALPHACLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.ALPHACLAW_SKIP_GMAIL_WATCHER;
    process.env.ALPHACLAW_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.ALPHACLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.ALPHACLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ALPHACLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
