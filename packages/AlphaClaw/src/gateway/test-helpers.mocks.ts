...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `alphaclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.ALPHACLAW_CONFIG_PATH = path.join(root, "alphaclaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "alphaclaw.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "alphaclaw-gateway-test"),
        ...fileDefaults,
