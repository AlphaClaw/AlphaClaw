...
This directory contains hooks that ship with AlphaClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.alphaclaw.ai/hooks/soul-evil
...
Hooks can be configured in `~/.alphaclaw/alphaclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'alphaclaw.*gateway' && pnpm alphaclaw gateway`
3. Enable the hook: `alphaclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.alphaclaw.ai/hooks
