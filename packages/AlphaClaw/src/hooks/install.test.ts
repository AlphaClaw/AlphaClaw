...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `alphaclaw-hook-install-${randomUUID()}`);
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      JSON.stringify({
        name: "@alphaclaw/zip-hooks",
        version: "0.0.1",
        alphaclaw: { hooks: ["./hooks/zip-hook"] },
      }),
    );
    zip.file(
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@alphaclaw/tar-hooks",
        version: "0.0.1",
        alphaclaw: { hooks: ["./hooks/tar-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("rejects hook packs with traversal-like ids", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@evil/..",
        version: "0.0.1",
        alphaclaw: { hooks: ["./hooks/evil-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("rejects hook packs with reserved ids", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@evil/.",
        version: "0.0.1",
        alphaclaw: { hooks: ["./hooks/reserved-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
