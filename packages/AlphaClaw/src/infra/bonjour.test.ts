...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.ALPHACLAW_MDNS_HOSTNAME = "test-host";
    process.env.ALPHACLAW_MDNS_HOSTNAME = "test-host";
    ...
    expect(gatewayCall?.[0]?.type).toBe("alphaclaw-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("alphaclaw (AlphaClaw)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("alphaclaw");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("alphaclaw.local");
