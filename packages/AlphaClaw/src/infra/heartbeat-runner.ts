...
function resolveActiveHoursTimezone(cfg: AlphaClawConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: AlphaClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: AlphaClawConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: AlphaClawConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: AlphaClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: AlphaClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: AlphaClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: AlphaClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
