...
import { ensureAlphaClawCliOnPath } from "./path-env.js";

describe("ensureAlphaClawCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling alphaclaw exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "alphaclaw-path-"));
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const cliPath = path.join(appBinDir, "alphaclaw");
      await fs.writeFile(cliPath, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(cliPath, 0o755);

      const originalPath = process.env.PATH;
      const originalFlag = process.env.ALPHACLAW_PATH_BOOTSTRAPPED;
      process.env.PATH = "/usr/bin";
      delete process.env.ALPHACLAW_PATH_BOOTSTRAPPED;
      try {
        ensureAlphaClawCliOnPath({
          execPath: cliPath,
          cwd: tmp,
          homeDir: tmp,
          platform: "darwin",
        ...
        if (originalFlag === undefined) {
          ...
          process.env.ALPHACLAW_PATH_BOOTSTRAPPED = originalFlag;
        }
      }
    } finally {
  ...
  it("prepends mise shims when available", async () => {
    ...
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const appCli = path.join(appBinDir, "alphaclaw");
      await fs.writeFile(appCli, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(appCli, 0o755);

      const localBinDir = path.join(tmp, "node_modules", ".bin");
      await fs.mkdir(localBinDir, { recursive: true });
      const localCli = path.join(localBinDir, "alphaclaw");
