...
function candidateBinDirs(opts: EnsureAlphaClawPathOpts): string[] {
  ...
  // Bundled macOS app: `alphaclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "alphaclaw");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/alphaclaw` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "alphaclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `alphaclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAlphaClawCliOnPath(opts: EnsureAlphaClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ALPHACLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.ALPHACLAW_PATH_BOOTSTRAPPED = "1";
