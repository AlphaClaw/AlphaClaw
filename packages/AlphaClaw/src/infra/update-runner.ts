...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alphaclaw-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "alphaclaw doctor",
        managerScriptArgs(manager, "alphaclaw", ["doctor", "--non-interactive"]),
        gitRoot,
        { ALPHACLAW_UPDATE_IN_PROGRESS: "1" },
      ),
    );
