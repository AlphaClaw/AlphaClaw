...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ALPHACLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*alphaclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "alphaclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "alphaclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "alphaclaw-gw");
  ...
  records.push(`_alphaclaw-gw._tcp IN PTR ${instanceLabel}._alphaclaw-gw._tcp`);
  records.push(`${instanceLabel}._alphaclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._alphaclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; alphaclaw-content-hash: ${contentHash}\n${contentBody}`;
}
