...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "ALPHACLAW_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "alphaclaw-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.ALPHACLAW_STATE_DIR = path.join(home, ".alphaclaw");
    ...
    await fs.mkdir(path.join(home, ".alphaclaw"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.alphaclaw${path.sep}media`);
