...
const execHostEnforced = process.env.ALPHACLAW_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.ALPHACLAW_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureAlphaClawCliOnPath({ pathEnv: process.env.PATH ?? "" });
