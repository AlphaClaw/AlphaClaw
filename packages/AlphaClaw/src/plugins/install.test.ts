...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `alphaclaw-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.alphaclaw/extensions and uses unscoped id", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "0.0.1",
        alphaclaw: { extensions: ["./dist/index.js"] },
      }),
      "utf-8",
    );
  ...
  it("rejects packages without alphaclaw.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@alphaclaw/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("alphaclaw.extensions");
  });
});
