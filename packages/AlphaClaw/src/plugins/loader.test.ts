...
import { loadAlphaClawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.ALPHACLAW_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `alphaclaw-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "alphaclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.ALPHACLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadAlphaClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.ALPHACLAW_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadAlphaClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadAlphaClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
