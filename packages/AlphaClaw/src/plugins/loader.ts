...
import { discoverAlphaClawPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AlphaClawPluginDefinition;
  register?: AlphaClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AlphaClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AlphaClawPluginDefinition;
...
export function loadAlphaClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverAlphaClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            "alphaclaw/plugin-sdk": pluginSdkAlias,
            // Legacy alias for backward compat with older plugins
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: AlphaClawPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as AlphaClawPluginModule;
    } catch (err) {
