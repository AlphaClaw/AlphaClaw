...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "alphaclaw-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "alphaclaw-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "alphaclaw-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "alphaclaw-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.alphaclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "alphaclaw-config-dir-"));
    try {
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/alphaclaw")).toBe(path.resolve(os.homedir(), "alphaclaw"));
  });
