...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `alphaclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".alphaclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          ALPHACLAW_SKIP_CHANNELS: "1",
          ALPHACLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
          ALPHACLAW_SKIP_CANVAS_HOST: "1",
        },
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        ALPHACLAW_GATEWAY_TOKEN: inst.gatewayToken,
        ALPHACLAW_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          ALPHACLAW_GATEWAY_PORT: String(gwA.port),
          ALPHACLAW_GATEWAY_TOKEN: gwA.gatewayToken,
          ALPHACLAW_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          ALPHACLAW_GATEWAY_PORT: String(gwB.port),
          ALPHACLAW_GATEWAY_TOKEN: gwB.gatewayToken,
          ALPHACLAW_GATEWAY_PASSWORD: "",
        }),
