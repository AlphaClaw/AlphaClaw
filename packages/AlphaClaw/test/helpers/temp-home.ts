...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("ALPHACLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.ALPHACLAW_STATE_DIR = path.join(base, ".alphaclaw");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "alphaclaw-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".alphaclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
