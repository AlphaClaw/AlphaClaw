...
import type { AlphaClawConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("alphaclaw-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("alphaclaw-bin-sherpa-");
      const modelDir = await makeTempDir("alphaclaw-sherpa-model-");
      ...
      const cfg: AlphaClawConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("alphaclaw-bin-whispercpp-");
      const modelDir = await makeTempDir("alphaclaw-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("alphaclaw-bin-gemini-");
      ...
      const cfg: AlphaClawConfig = { tools: { media: { image: {} } } };
