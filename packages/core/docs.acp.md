...
`alphaclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
...
3. Point the IDE to run `alphaclaw acp` over stdio.
...
- ACP client spawns `alphaclaw acp` and speaks ACP messages over stdio.
...
`alphaclaw acp` resolves the Gateway URL and auth from CLI flags or config:
