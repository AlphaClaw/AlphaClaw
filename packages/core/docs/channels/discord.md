...
- Talk to AlphaClaw via Discord DMs or guild channels.
...
3. Configure AlphaClaw with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `alphaclaw pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running AlphaClaw in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents AlphaClaw needs
...
Discord uses numeric ids everywhere; AlphaClaw config prefers ids.
...
- Owner hint: when a per-guild or per-channel `users` allowlist matches the sender, AlphaClaw treats that sender as the owner in the system prompt. For a global owner across channels, set `commands.ownerAllowFrom`.
...
- First: run `alphaclaw doctor` and `alphaclaw channels status --probe` (actionable warnings + quick audits).
...
When enabled, AlphaClaw uses the member identity for allowlists and labels the
...
- On startup, AlphaClaw resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:

- The registered commands mirror AlphaClaw’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; AlphaClaw enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`alphaclaw gateway --force`) after confirming no other processes own the Discord session.
