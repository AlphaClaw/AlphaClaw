...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects AlphaClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
If you just installed AlphaClaw, run the wizard:
...
2. Configuring app credentials in AlphaClaw
...
1. You already ran `alphaclaw channels add` for Feishu
2. The gateway is running (`alphaclaw gateway status`)
...
```bash
alphaclaw pairing approve feishu <CODE>
```
...
2. Run `alphaclaw logs --follow` and look for `chat_id`
...
2. Run `alphaclaw logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `alphaclaw gateway status`  | Show gateway status           |
| `alphaclaw gateway install` | Install/start gateway service |
| `alphaclaw gateway stop`    | Stop gateway service          |
| `alphaclaw gateway restart` | Restart gateway service       |
| `alphaclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `alphaclaw logs --follow`
...
5. Ensure the gateway is running: `alphaclaw gateway status`
6. Check logs: `alphaclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.alphaclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.alphaclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
