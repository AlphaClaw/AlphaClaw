...
```bash
alphaclaw plugins install @alphaclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
alphaclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "AlphaClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `alphaclaw pairing list nextcloud-talk`
  - `alphaclaw pairing approve nextcloud-talk <CODE>`
