...
Tlon is a decentralized messenger built on Urbit. AlphaClaw connects to your Urbit ship and can
...
```bash
alphaclaw plugins install @alphaclaw/tlon
```

Local checkout (when running from a git repo):

```bash
alphaclaw plugins install ./extensions/tlon
```
...
Use these with `alphaclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, AlphaClaw replies in-thread.
