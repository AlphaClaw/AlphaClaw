...
read_when:
  - Setting up Twitch chat integration for AlphaClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. AlphaClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
alphaclaw plugins install @alphaclaw/twitch
```

Local checkout (when running from a git repo):

```bash
alphaclaw plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "alphaclaw", // Bot's Twitch account
