...
- `/usage cost` in chats: local cost summary aggregated from AlphaClaw session logs.
- CLI: `alphaclaw status --usage` prints a full per-provider breakdown.
- CLI: `alphaclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
