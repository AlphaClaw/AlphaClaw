...
- During heartbeat runs, AlphaClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `alphaclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, AlphaClaw skips the heartbeat run entirely (no model call).
...
```bash
alphaclaw system event --text "Check for urgent follow-ups" --mode now
```
