...
- Replaces the legacy `gateway` command. CLI entry point: `alphaclaw gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
alphaclaw gateway --port 18789
# for full debug/trace logs in stdio:
alphaclaw gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
alphaclaw gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.alphaclaw/alphaclaw.json` (or `OPENCLAW_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__alphaclaw__/canvas/` from `~/.alphaclaw/workspace/canvas`. Disable with `canvasHost.enabled=false` or `OPENCLAW_SKIP_CANVAS_HOST=1`.
...
- macOS: `bot.molt.<profile>` (legacy `com.alphaclaw.*` may still exist)
- Linux: `alphaclaw-gateway-<profile>.service`
- Windows: `AlphaClaw Gateway (<profile>)`

Install metadata is embedded in the service config:

- `OPENCLAW_SERVICE_MARKER=alphaclaw`
...
```bash
alphaclaw --dev setup
alphaclaw --dev gateway --allow-unconfigured
# then target the dev instance:
alphaclaw --dev status
alphaclaw --dev health
```

Defaults (can be overridden via env/flags/config):

- `OPENCLAW_STATE_DIR=~/.alphaclaw-dev`
- `OPENCLAW_CONFIG_PATH=~/.alphaclaw-dev/alphaclaw.json`
- `OPENCLAW_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.alphaclaw/workspace-dev` when you run `setup`/`onboard` under `--dev`.
...
```bash
alphaclaw --profile main gateway install
alphaclaw --profile rescue gateway install
```

Example:

```bash
OPENCLAW_CONFIG_PATH=~/.alphaclaw/a.json OPENCLAW_STATE_DIR=~/.alphaclaw-a alphaclaw gateway --port 19001
OPENCLAW_CONFIG_PATH=~/.alphaclaw/b.json OPENCLAW_STATE_DIR=~/.alphaclaw-b alphaclaw gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `alphaclaw health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `alphaclaw`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `alphaclaw gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.alphaclaw.*` is cleaned up).
  - `alphaclaw doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are AlphaClaw profile services.
  ...
  - Cleanup: `alphaclaw gateway uninstall` (current service) and `alphaclaw doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `alphaclaw gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- AlphaClaw.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.alphaclaw.*` labels still unload cleanly).
- To stop it cleanly, use `alphaclaw gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `alphaclaw gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `alphaclaw gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

AlphaClaw installs a **systemd user service** by default on Linux/WSL2. We
...
`alphaclaw gateway install` writes the user unit. `alphaclaw doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/alphaclaw-gateway[-<profile>].service`:

```
[Unit]
Description=AlphaClaw Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/alphaclaw gateway --port 18789
Restart=always
RestartSec=5
...
```
systemctl --user enable --now alphaclaw-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/alphaclaw-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now alphaclaw-gateway[-<profile>].service
```
...
- `alphaclaw gateway health|status` — request health/status over the Gateway WS.
- `alphaclaw message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `alphaclaw agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `alphaclaw gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `alphaclaw gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `alphaclaw gateway` and the legacy TCP control port.
