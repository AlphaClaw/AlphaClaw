...
## Quick check: `alphaclaw security audit`
...
```bash
alphaclaw security audit
alphaclaw security audit --deep
alphaclaw security audit --fix
```
...
- Tighten local perms (`~/.alphaclaw` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

AlphaClaw is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, AlphaClaw also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.alphaclaw/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.alphaclaw/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.alphaclaw/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.alphaclaw/credentials/oauth.json`
...
`alphaclaw security audit` warns when this setting is enabled.
...
AlphaClaw stores session transcripts on disk under `~/.alphaclaw/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.alphaclaw` (see the audit section below). If you need
...
AlphaClaw can refresh the skills list mid-session:
...
- If you install plugins from npm (`alphaclaw plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.alphaclaw/extensions/<pluginId>/` (or `$OPENCLAW_STATE_DIR/extensions/<pluginId>/`).
  - AlphaClaw uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
alphaclaw pairing approve <channel> <code>
```
...
By default, AlphaClaw routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
AlphaClaw has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.alphaclaw/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.alphaclaw or your logs.”
...
4. **Re-run audit**
   - `alphaclaw security audit --deep` and confirm the report is clean.
...
- `~/.alphaclaw/alphaclaw.json`: `600` (user read/write only)
- `~/.alphaclaw`: `700` (user only)

`alphaclaw doctor` can warn and offer to tighten these permissions.
...
The Gateway broadcasts its presence via mDNS (`_alphaclaw-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
Doctor can generate one for you: `alphaclaw doctor --generate-gateway-token`.
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), AlphaClaw
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. AlphaClaw verifies the identity by resolving the
...
- AlphaClaw will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.alphaclaw/` (or `$OPENCLAW_STATE_DIR/`) may contain secrets or private data:

- `alphaclaw.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `alphaclaw status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@alphaclaw", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.alphaclaw/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `alphaclaw` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only AlphaClaw clients can connect.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `alphaclaw gateway` process.
...
1. Check Gateway logs: `/tmp/alphaclaw/alphaclaw-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.alphaclaw/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + AlphaClaw version
...
Found a vulnerability in AlphaClaw? Please report responsibly:

1. Email: security@alphaclaw.ai
