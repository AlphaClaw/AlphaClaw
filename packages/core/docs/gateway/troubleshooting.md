---
summary: "Quick troubleshooting guide for common AlphaClaw failures"
read_when:
...
When AlphaClaw misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `alphaclaw status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `alphaclaw status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `alphaclaw status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `alphaclaw gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `alphaclaw channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `alphaclaw gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `alphaclaw logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `alphaclaw status --all` (it redacts tokens). If you paste `alphaclaw status`, consider setting `OPENCLAW_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  alphaclaw models auth setup-token --provider anthropic
  ```
...
```bash
alphaclaw models auth paste-token --provider anthropic
alphaclaw models status
```
...
- Preferred: `alphaclaw logs --follow`
- File logs (always): `/tmp/alphaclaw/alphaclaw-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$OPENCLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u alphaclaw-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  alphaclaw config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  alphaclaw config set gateway.mode remote
  alphaclaw config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `alphaclaw setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.alphaclaw/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `alphaclaw doctor`
...
- then run `alphaclaw sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `alphaclaw gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `alphaclaw gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `alphaclaw gateway status` and `alphaclaw doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
...
**If `alphaclaw gateway status` shows a config mismatch**
...
- Fix: rerun `alphaclaw gateway install --force` from the same `--profile` / `OPENCLAW_STATE_DIR` you want the service to use.

**If `alphaclaw gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `alphaclaw doctor` to update it (or `alphaclaw gateway install --force` for a full rewrite).
...
**If `alphaclaw gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/alphaclaw` on disk.
...
Symptoms: `pwd` or file tools show `~/.alphaclaw/sandboxes/...` even though you
...
AlphaClaw intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `alphaclaw models list` or
  `alphaclaw models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${OPENCLAW_CONFIG_PATH:-$HOME/.alphaclaw/alphaclaw.json}"
```
...
tail -f "$(ls -t /tmp/alphaclaw/alphaclaw-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
alphaclaw pairing list <channel>
```
...
```bash
alphaclaw logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.alphaclaw/agents/<agentId>/sessions/
```
...
alphaclaw logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
alphaclaw gateway --verbose
```

If you’re logged out / unlinked:

```bash
alphaclaw channels logout
trash "${OPENCLAW_STATE_DIR:-$HOME/.alphaclaw}/credentials" # if logout can't cleanly remove everything
alphaclaw channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/alphaclaw/alphaclaw-*.log | head -1)" | tail -20
```

### High Memory Usage

AlphaClaw keeps conversation history in memory.
...
AlphaClaw now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
```bash
alphaclaw doctor
alphaclaw doctor --fix
```

Notes:

- `alphaclaw doctor` reports every invalid entry.
- `alphaclaw doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `alphaclaw logs`, `alphaclaw health`, `alphaclaw status`, `alphaclaw gateway status`, and `alphaclaw gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/alphaclaw/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `alphaclaw models status` (CLI).
...
```bash
curl -fsSL https://alphaclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
```
...
5. Run `alphaclaw channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. AlphaClaw scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/alphaclaw/alphaclaw/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.alphaclaw.* still works)
```
...
Ensure the global `alphaclaw` CLI is installed and matches the app version:

```bash
alphaclaw --version
npm install -g alphaclaw@<version>
```
...
#   ${OPENCLAW_CONFIG_PATH:-$HOME/.alphaclaw/alphaclaw.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
alphaclaw gateway --verbose
alphaclaw channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/alphaclaw/alphaclaw-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$OPENCLAW_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.alphaclaw/logs/...`; profiles use `~/.alphaclaw-<profile>/logs/...`)<br />Linux: `journalctl --user -u alphaclaw-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "AlphaClaw Gateway (<profile>)" /V /FO LIST` |
...
tail -20 /tmp/alphaclaw/alphaclaw-*.log
```
...
trash "${OPENCLAW_STATE_DIR:-$HOME/.alphaclaw}"
alphaclaw channels login         # re-pair WhatsApp
alphaclaw gateway restart           # or: alphaclaw gateway
```
...
1. Check logs first: `/tmp/alphaclaw/` (default: `alphaclaw-YYYY-MM-DD.log`, or your configured `logging.file`)
