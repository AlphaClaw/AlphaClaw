...
read_when:
  - "You installed AlphaClaw but `alphaclaw` is “command not found”"
...
AlphaClaw’s runtime baseline is **Node 22+**.

If you can run `npm install -g alphaclaw@latest` but later see `alphaclaw: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
