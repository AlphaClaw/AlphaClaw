---
summary: "Use MiniMax M2.1 in AlphaClaw"
read_when:
  - You want MiniMax models in AlphaClaw
...
```bash
alphaclaw plugins enable minimax-portal-auth  # skip if already loaded.
alphaclaw gateway restart  # restart if gateway is already running
alphaclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/alphaclaw/alphaclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `alphaclaw.json`:
...
- Use `alphaclaw models list` and `alphaclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `alphaclaw configure` and selecting **MiniMax M2.1**, or
