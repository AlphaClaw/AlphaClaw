...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.alphaclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.alphaclaw/node.json` (node id + pairing token).
...
Path: `~/.alphaclaw/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.alphaclaw/exec-approvals.sock",
...
- Unix socket at `~/.alphaclaw/exec-approvals.sock` (0600).
