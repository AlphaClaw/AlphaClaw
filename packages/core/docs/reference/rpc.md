...
AlphaClaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- AlphaClaw owns lifecycle when `channels.signal.autoStart=true`.
...
- AlphaClaw spawns `imsg rpc` as a child process (legacy iMessage integration).
