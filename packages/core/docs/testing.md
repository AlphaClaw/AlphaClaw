...
AlphaClaw has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
### Layer 2: Gateway + dev agent smoke (what “@alphaclaw” actually does)
...
```bash
alphaclaw models auth paste-token --provider anthropic --profile-id anthropic:setup-token-test
...
- Gemini API vs Gemini CLI:
  - API: AlphaClaw calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: AlphaClaw shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
- OpenRouter: `openrouter/...` (hundreds of models; use `alphaclaw models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `alphaclaw models list` / model selection.

- Profile store: `~/.alphaclaw/credentials/` (preferred; what “profile keys” means in the tests)
...
- `OPENCLAW_CONFIG_DIR=...` (default: `~/.alphaclaw`) mounted to `/home/node/.alphaclaw`
- `OPENCLAW_WORKSPACE_DIR=...` (default: `~/.alphaclaw/workspace`) mounted to `/home/node/.alphaclaw/workspace`
