---
summary: "Chrome extension: let AlphaClaw drive your existing Chrome tab"
read_when:
...
The AlphaClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate alphaclaw-managed Chrome profile.
...
AlphaClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the AlphaClaw release (npm package) as static files. There is no separate “build” step.

After upgrading AlphaClaw:

- Re-run `alphaclaw browser extension install` to refresh the installed files under your AlphaClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

AlphaClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want AlphaClaw to control.
...
- `ON`: attached; AlphaClaw can drive that tab.
...
Debugging: `alphaclaw sandbox explain`
...
`alphaclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `alphaclaw browser extension install` first to copy the extension to a stable location under your AlphaClaw state directory.
...
- **This is not isolated** like the dedicated alphaclaw-managed profile.
