...
```bash
alphaclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `alphaclaw tui --deliver`
...
- Check the Gateway logs: `alphaclaw logs --follow`.
- Confirm the agent can run: `alphaclaw status` and `alphaclaw models status`.
...
- No agents in picker: check `alphaclaw agents list` and your routing config.
