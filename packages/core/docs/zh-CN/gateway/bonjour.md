...
AlphaClaw 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现
...
2. 在专用区域下发布 `_alphaclaw-gw._tcp` 的 DNS‑SD 记录
   （示例：`alphaclaw.internal.`）。
3. 配置 Tailscale **分割 DNS**，使你选择的域名通过该
   DNS 服务器为客户端（包括 iOS）解析。

AlphaClaw 支持任何发现域名；`alphaclaw.internal.` 只是一个示例。
...
- 从 `~/.alphaclaw/dns/<domain>.db` 提供你选择的域名服务（示例：`alphaclaw.internal.`）
...
你的发现域名中浏览 `_alphaclaw-gw._tcp`，无需多播。
...
- 在 `~/.alphaclaw/alphaclaw.json` 中设置 `gateway.bind: "tailnet"`。
- 重启 Gateway 网关（或重启 macOS 菜单栏应用）。

## 什么在广播

只有 Gateway 网关广播 `_alphaclaw-gw._tcp`。

## 服务类型

- `_alphaclaw-gw._tcp` — Gateway 网关传输信标（被 macOS/iOS/Android 节点使用）。
...
- `transport=gateway`
- `cliPath=<path>`（可选；可运行的 `alphaclaw` 入口点的绝对路径）
...
iOS 节点使用 `NWBrowser` 来发现 `_alphaclaw-gw._tcp`。
...
- `~/.alphaclaw/alphaclaw.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
