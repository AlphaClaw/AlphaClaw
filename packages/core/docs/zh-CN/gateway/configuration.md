...
read_when:
  - 添加或修改配置字段时
summary: ~/.alphaclaw/alphaclaw.json 的所有配置选项及示例
title: 配置
x-i18n:
...
AlphaClaw 从 `~/.alphaclaw/alphaclaw.json` 读取可选的 **JSON5** 配置（支持注释和尾逗号）。

如果文件不存在，AlphaClaw 使用安全的默认值（内置 Pi 智能体 + 按发送者分会话 + 工作区 `~/.alphaclaw/workspace`）。通常只在以下情况需要配置：
...
AlphaClaw 只接受完全匹配 schema 的配置。
未知键、类型错误或无效值会导致 Gateway 网关 **拒绝启动**以确保安全。

验证失败时：

- Gateway 网关不会启动。
- 只允许诊断命令（例如：`alphaclaw doctor`、`alphaclaw logs`、`alphaclaw health`、`alphaclaw status`、`alphaclaw service`、`alphaclaw help`）。
- 运行 `alphaclaw doctor` 查看具体问题。
- 运行 `alphaclaw doctor --fix`（或 `--yes`）应用迁移/修复。
...
请使用 `config.patch` 或 `alphaclaw config set`。请备份 `~/.alphaclaw/alphaclaw.json`。
...
{ "$include": "/etc/alphaclaw/base.json5" } // 绝对路径
...
AlphaClaw 从父进程（shell、launchd/systemd、CI 等）读取环境变量。

此外，它还会加载：

- 当前工作目录中的 `.env`（如果存在）
- `~/.alphaclaw/.env`（即 `$OPENCLAW_STATE_DIR/.env`）作为全局回退 `.env`
...
可选便利功能：如果启用且预期键均未设置，AlphaClaw 会运行你的登录 shell 并仅导入缺失的预期键（不会覆盖）。
...
AlphaClaw 在以下位置存储**每个智能体的**认证配置文件（OAuth + API 密钥）：

- `<agentDir>/auth-profiles.json`（默认：`~/.alphaclaw/agents/<agentId>/agent/auth-profiles.json`）

另请参阅：[/concepts/oauth](/concepts/oauth)

旧版 OAuth 导入：

- `~/.alphaclaw/credentials/oauth.json`（或 `$OPENCLAW_STATE_DIR/credentials/oauth.json`）
...
- `~/.alphaclaw/agent/*`（由 `alphaclaw doctor` 迁移到 `~/.alphaclaw/agents/<defaultAgentId>/agent/*`）
...
首次使用时，AlphaClaw 会将 `oauth.json` 条目导入到 `auth-profiles.json` 中。
...
如果设置了，AlphaClaw 会推导默认值（仅在你未明确设置时）：
...
- 默认日志文件：`/tmp/alphaclaw/alphaclaw-YYYY-MM-DD.log`
- 如需稳定路径，将 `logging.file` 设为 `/tmp/alphaclaw/alphaclaw.log`。
- 控制台输出可通过以下方式单独调整：
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // 可选覆盖。默认：~/.alphaclaw/credentials/whatsapp/biz
...
- 旧版单账号 Baileys 认证目录由 `alphaclaw doctor` 迁移到 `whatsapp/default`。
...
- `agents.list[]`：每智能体覆盖。
  ...
  - `name`：智能体的显示名称。
  - `workspace`：默认 `~/.alphaclaw/workspace-<agentId>`（对于 `main`，回退到 `agents.defaults.workspace`）。
  - `agentDir`：默认 `~/.alphaclaw/agents/<agentId>/agent`。
...
- `commands.config: true` 启用 `/config`（读写 `alphaclaw.json`）。
...
AlphaClaw 仅在存在 `channels.telegram` 配置段时启动 Telegram。机器人 token 从 `channels.telegram.botToken`（或 `channels.telegram.tokenFile`）解析，`TELEGRAM_BOT_TOKEN` 作为默认账号的回退。
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["alphaclaw-dm"], // 可选群组私聊白名单
      },
      guilds: {
        "123456789012345678": {
          // 服务器 id（推荐）或 slug
...
AlphaClaw 仅在存在 `channels.discord` 配置段时启动 Discord。token 从 `channels.discord.token` 解析，`DISCORD_BOT_TOKEN` 作为默认账号的回退（除非 `channels.discord.enabled` 为 `false`）。在为 cron/CLI 命令指定投递目标时，使用 `user:<id>`（私聊）或 `channel:<id>`（服务器频道）；裸数字 ID 有歧义会被拒绝。
...
AlphaClaw 在提供商启用且两个 token 都已设置时启动 Slack（通过配置或 `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`）。在为 cron/CLI 命令指定投递目标时使用 `user:<id>`（私聊）或 `channel:<id>`。
...
请先安装：`alphaclaw plugins install @alphaclaw/mattermost`（或从 git checkout 使用 `./extensions/mattermost`）。
...
AlphaClaw 在账号已配置（机器人 token + 基础 URL）且已启用时启动 Mattermost。token + 基础 URL 从 `channels.mattermost.botToken` + `channels.mattermost.baseUrl` 或默认账号的 `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` 解析（除非 `channels.mattermost.enabled` 为 `false`）。
...
AlphaClaw 会生成 `imsg rpc`（通过 stdio 的 JSON-RPC）。无需守护进程或端口。
...
在系统提示的 Runtime 行中显示的可选仓库根目录。如果未设置，AlphaClaw 会从工作区（和当前工作目录）向上查找 `.git` 目录进行检测。路径必须存在才能使用。
...
当文件超过此限制时，AlphaClaw 会记录警告并注入带标记的头尾截断内容。
...
设置用户时区用于**系统提示上下文**（不用于消息信封中的时间戳）。如果未设置，AlphaClaw 在运行时使用主机时区。
...
如果未设置 `messages.responsePrefix`，默认不应用前缀。WhatsApp 自聊天回复是例外：它们在设置时默认为 `[{identity.name}]`，否则为 `[alphaclaw]`，以保持同一手机上的对话可读性。
...
WhatsApp 入站前缀通过 `channels.whatsapp.messagePrefix` 配置（已弃用：`messages.messagePrefix`）。默认保持**不变**：当 `channels.whatsapp.allowFrom` 为空时为 `"[alphaclaw]"`，否则为 `""`（无前缀）。使用 `"[alphaclaw]"` 时，如果路由的智能体设置了 `identity.name`，AlphaClaw 会改用 `[{identity.name}]`。
...
为出站回复启用文字转语音。开启后，AlphaClaw 使用 ElevenLabs 或 OpenAI 生成音频并附加到回复中。Telegram 使用 Opus 语音消息；其他渠道发送 MP3 音频。
...
AlphaClaw 还内置了一些别名快捷方式。默认值仅在模型已存在于 `agents.defaults.models` 中时才应用：
...
如果省略提供商，AlphaClaw 目前假定 `anthropic` 作为临时弃用回退。
...
- `tools.web.search.apiKey`（推荐：通过 `alphaclaw configure --section web` 设置，或使用 `BRAVE_API_KEY` 环境变量）
...
- `group:nodes`：`nodes`
- `group:alphaclaw`：所有内置 AlphaClaw 工具（不包含提供商插件）
...
- 智能体工作区访问：`workspaceAccess: "none"`（默认）
  - `"none"`：在 `~/.alphaclaw/sandboxes` 下使用每范围的沙箱工作区
...
AlphaClaw 使用 **pi-coding-agent** 模型目录。你可以通过编写
`~/.alphaclaw/agents/<agentId>/agent/models.json` 或在 AlphaClaw 配置中的 `models.providers` 下定义相同的 schema 来添加自定义提供商（LiteLLM、本地 OpenAI 兼容服务器、Anthropic 代理等）。
按提供商的概述 + 示例：[/concepts/model-providers](/concepts/model-providers)。

当存在 `models.providers` 时，AlphaClaw 在启动时将 `models.json` 写入/合并到
`~/.alphaclaw/agents/<agentId>/agent/`：
...
OpenCode Zen 是一个具有每模型端点的多模型网关。AlphaClaw 使用
...
- 快捷方式：`alphaclaw onboard --auth-choice opencode-zen`。
...
快捷方式：`alphaclaw onboard --auth-choice zai-api-key`。
...
- 在环境中设置 `MOONSHOT_API_KEY` 或使用 `alphaclaw onboard --auth-choice moonshot-api-key`。
...
- 在环境中设置 `KIMI_API_KEY` 或使用 `alphaclaw onboard --auth-choice kimi-code-api-key`。
...
- 设置 `SYNTHETIC_API_KEY` 或使用 `alphaclaw onboard --auth-choice synthetic-api-key`。
...
- 设置 `MINIMAX_API_KEY` 环境变量或使用 `alphaclaw onboard --auth-choice minimax-api`。
...
- 如果你希望 `models.json` 存储在其他位置，请使用 `OPENCLAW_AGENT_DIR`（或 `PI_CODING_AGENT_DIR`）覆盖智能体配置根目录（默认：`~/.alphaclaw/agents/main/agent`）。
...
```json5
{
  session: {
    ...
    // 默认已按智能体存储在 ~/.alphaclaw/agents/<agentId>/sessions/sessions.json
...
- `resetByType`：`dm`、`group` 和 `thread` 的每会话覆盖。
  - 如果你只设置了旧版 `session.idleMinutes` 而没有任何 `reset`/`resetByType`，AlphaClaw 保持仅空闲模式以向后兼容。
...
控制内置白名单、安装偏好、额外 Skills 文件夹和每 Skills 覆盖。适用于**内置**Skills 和 `~/.alphaclaw/skills`（工作区 Skills 在名称冲突时仍然优先）。
...
控制插件发现、允许/拒绝和每插件配置。插件从 `~/.alphaclaw/extensions`、`<workspace>/.alphaclaw/extensions` 以及任何 `plugins.load.paths` 条目加载。**配置更改需要重启 Gateway 网关。**
...
### `browser`（AlphaClaw 管理的浏览器）

AlphaClaw 可以为 AlphaClaw 启动一个**专用、隔离的** Chrome/Brave/Edge/Chromium 实例并暴露一个小型 local loopback 控制服务。
...
- 配置文件颜色：`#FF4500`（龙虾橙）
- 注意：控制服务器由运行中的 Gateway 网关（AlphaClaw.app 菜单栏或 `alphaclaw gateway`）启动。
...
- 示例：`"/ui"`、`"/alphaclaw"`、`"/apps/alphaclaw"`。
- 默认：根路径（`/`）（不变）。
...
- 当连接来自这些 IP 之一时，AlphaClaw 使用 `x-forwarded-for`（或 `x-real-ip`）来确定客户端 IP，用于本地配对检查和 HTTP 认证/本地检查。
- 仅列出你完全控制的代理，并确保它们**覆盖**传入的 `x-forwarded-for`。

说明：

- `alphaclaw gateway` 拒绝启动，除非 `gateway.mode` 设为 `local`（或你传递了覆盖标志）。
...
- `gateway.auth.allowTailscale` 允许 Tailscale Serve 身份头部
  ...
  `x-forwarded-proto` 和 `x-forwarded-host` 时满足认证。AlphaClaw 在接受之前
...
- AlphaClaw.app 监视 `~/.alphaclaw/alphaclaw.json`，当 `gateway.mode` 或 `gateway.remote.url` 变更时实时切换模式。
...
Gateway 网关监视 `~/.alphaclaw/alphaclaw.json`（或 `OPENCLAW_CONFIG_PATH`）并自动应用更改。
...
- `~/.alphaclaw/alphaclaw.json`（或 `OPENCLAW_CONFIG_PATH`）
...
- `alphaclaw --dev …` → 使用 `~/.alphaclaw-dev` + 端口从基础 `19001` 偏移
- `alphaclaw --profile <name> …` → 使用 `~/.alphaclaw-<name>`（端口通过配置/环境变量/标志）
...
Gmail 辅助配置（由 `alphaclaw webhooks gmail setup` / `run` 使用）：
...
注意：当 `tailscale.mode` 开启时，AlphaClaw 将 `serve.path` 默认为 `/`，以便
...
默认根目录：`~/.alphaclaw/workspace/canvas`
默认端口：`18793`（选择此端口以避免 AlphaClaw 浏览器 CDP 端口 `18792`）
...
- 监视目录并通过 `/__alphaclaw__/ws` 的 WebSocket 端点广播重载
- 目录为空时自动创建起始 `index.html`（以便你立即看到内容）
- 同时在 `/__alphaclaw__/a2ui/` 提供 A2UI，并作为 `canvasHostUrl` 通告给节点
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // 省略时使用 ~/.alphaclaw/bridge/tls/bridge-{cert,key}.pem。
...
控制 LAN mDNS 发现广播（`_alphaclaw-gw._tcp`）。

- `minimal`（默认）：从 TXT 记录中省略 `cliPath` + `sshPort`
- `full`：在 TXT 记录中包含 `cliPath` + `sshPort`
- `off`：完全禁用 mDNS 广播
- 主机名：默认为 `alphaclaw`（通告 `alphaclaw.local`）。通过 `OPENCLAW_MDNS_HOSTNAME` 覆盖。
...
启用后，Gateway 网关在 `~/.alphaclaw/dns/` 下使用配置的发现域（示例：`alphaclaw.internal.`）为 `_alphaclaw-gw._tcp` 写入单播 DNS-SD 区域。
