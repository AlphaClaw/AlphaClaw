...
- `~/Library/Application Support/AlphaClaw/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `alphaclaw-canvas://<session>/<path>`

示例：

- `alphaclaw-canvas://main/` → `<canvasRoot>/main/index.html`
- `alphaclaw-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `alphaclaw-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
alphaclaw nodes canvas present --node <id>
alphaclaw nodes canvas navigate --node <id> --url "/"
alphaclaw nodes canvas eval --node <id> --js "document.title"
alphaclaw nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__alphaclaw__/a2ui/
```
...
alphaclaw nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
alphaclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "alphaclaw://agent?message=Review%20this%20design";
```
