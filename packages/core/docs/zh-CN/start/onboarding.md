...
read_when:
  - 设计 macOS 新手引导助手
  - 实现认证或身份设置
summary: AlphaClaw 的首次运行新手引导流程（macOS 应用）
title: 新手引导
x-i18n:
...
- 打开浏览器进行 OAuth（PKCE）
- 要求用户粘贴 `code#state` 值
- 将凭证写入 `~/.alphaclaw/credentials/oauth.json`
...
应用可以通过 npm/pnpm 安装全局 `alphaclaw` CLI，以便终端工作流和 launchd 任务开箱即用。
...
在首次智能体运行时，AlphaClaw 会引导一个工作区（默认 `~/.alphaclaw/workspace`）：
...
```bash
alphaclaw webhooks gmail setup --account you@gmail.com
```
...
- `~/.alphaclaw/credentials/oauth.json`
