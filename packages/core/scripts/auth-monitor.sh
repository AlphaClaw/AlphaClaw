...
#
# Suggested cron: */30 * * * * /home/admin/alphaclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send AlphaClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., alphaclaw-alerts)
...
STATE_FILE="$HOME/.alphaclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via AlphaClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use alphaclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via AlphaClaw to $NOTIFY_PHONE..."
            alphaclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! AlphaClaw is down. Run: ssh l36 '~/alphaclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
