...
show_usage() {
    cat << EOF
clawlog - AlphaClaw Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View AlphaClaw logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    AlphaClaw logs flow through the macOS unified log (subsystem: ai.alphaclaw).
