...
IMAGE_NAME="alphaclaw-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/alphaclaw-bin:$PATH"
  mkdir -p /tmp/alphaclaw-bin

  cat > /tmp/alphaclaw-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/alphaclaw-bin/systemctl

  cat > /tmp/alphaclaw-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/alphaclaw-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/alphaclaw"
  npm_entry="/tmp/npm-prefix/lib/node_modules/alphaclaw/dist/index.js"
  git_entry="/app/dist/index.js"
  git_cli="/app/alphaclaw.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/alphaclaw-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/alphaclaw-gateway.service"
    if [ ! -f "$unit_path" ]; then
