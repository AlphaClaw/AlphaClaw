...
IMAGE_NAME="alphaclaw-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/alphaclaw-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/alphaclaw-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/alphaclaw-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/alphaclaw-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/alphaclaw-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.alphaclaw/workspace"
    config_path="$HOME/.alphaclaw/alphaclaw.json"
    sessions_dir="$HOME/.alphaclaw/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.alphaclaw/alphaclaw.json" <<'"'"'JSON'"'"'
{
