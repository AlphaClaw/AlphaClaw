...
# Build and bundle AlphaClaw into a minimal .app we can open.
# Outputs to dist/AlphaClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/AlphaClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="AlphaClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.alphaclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/alphaclaw/alphaclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/AlphaClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AlphaClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AlphaClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/AlphaClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/AlphaClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/AlphaClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/AlphaClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/AlphaClaw/Resources/AlphaClaw.icns" "$APP_ROOT/Contents/Resources/AlphaClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/AlphaClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying AlphaClawKit resources"
OPENCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AlphaClawKit_AlphaClawKit.bundle"
if [ -d "$OPENCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AlphaClawKit_AlphaClawKit.bundle"
  cp -R "$OPENCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AlphaClawKit_AlphaClawKit.bundle"
else
  echo "WARN: AlphaClawKit resource bundle not found at $OPENCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running AlphaClaw"
killall -q AlphaClaw 2>/dev/null || true
