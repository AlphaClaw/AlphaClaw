#!/bin/bash
# Setup AlphaClaw Auth Management System
...
echo "=== AlphaClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'alphaclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  AlphaClaw message: Send warning via AlphaClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/alphaclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/alphaclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/alphaclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now alphaclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status alphaclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
