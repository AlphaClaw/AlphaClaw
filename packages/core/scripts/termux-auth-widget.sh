...
# AlphaClaw Auth Widget for Termux
...
SERVER="${ALPHACLAW_SERVER:-${ALPHACLAW_SERVER:-l36}}"

# Check auth status
termux-toast "Checking AlphaClaw auth..."

STATUS=$(ssh "$SERVER" '$HOME/alphaclaw/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/alphaclaw/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|ALPHACLAW_EXPIRING|ALPHACLAW_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/alphaclaw/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|ALPHACLAW_EXPIRED|ALPHACLAW_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: AlphaClaw won't work until re-auth"
                ;;
        esac
        ;;
