...
const bunRunner = process.platform === "win32" ? "bunx.cmd" : "bunx";
...
const shardOverride = Number.parseInt(process.env.ALPHACLAW_TEST_SHARDS ?? "", 10);
...
const windowsCiArgs = isWindowsCi
  ...
  : [];
const overrideWorkers = Number.parseInt(process.env.ALPHACLAW_TEST_WORKERS ?? "", 10);
...
const runOnce = (entry, extraArgs = []) =>
  new Promise((resolve) => {
    ...
    const child = spawn(bunRunner, args, {
      stdio: "inherit",
