...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "alphaclaw.sandbox=1");
  args.push("--label", `alphaclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `alphaclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `alphaclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("alphaclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`alphaclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`alphaclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("alphaclaw sandbox recreate --all");
}
