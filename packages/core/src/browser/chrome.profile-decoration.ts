...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".alphaclaw-profile-decorated");
}
 ...
 */
export function decorateAlphaClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ALPHACLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ALPHACLAW_BROWSER_COLOR).toUpperCase();
