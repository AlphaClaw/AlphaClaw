import type { BrowserConfig, BrowserProfileConfig, AlphaClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_ALPHACLAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_ALPHACLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "alphaclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ALPHACLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ALPHACLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an AlphaClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for alphaclaw/alphaclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: AlphaClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ALPHACLAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "alphaclaw";
