...
function setSlackDmPolicy(cfg: AlphaClawConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "AlphaClaw";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for AlphaClaw`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: AlphaClawConfig, allowFrom: string[]): AlphaClawConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
