...
function setWhatsAppDmPolicy(cfg: AlphaClawConfig, dmPolicy: DmPolicy): AlphaClawConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: AlphaClawConfig, allowFrom?: string[]): AlphaClawConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: AlphaClawConfig, selfChatMode: boolean): AlphaClawConfig {
...
async function detectWhatsAppLinked(cfg: AlphaClawConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so AlphaClaw can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for AlphaClaw" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("alphaclaw channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
