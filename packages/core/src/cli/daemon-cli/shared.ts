...
const SAFE_DAEMON_ENV_KEYS = [
  "ALPHACLAW_PROFILE",
  "ALPHACLAW_STATE_DIR",
  "ALPHACLAW_CONFIG_PATH",
  "ALPHACLAW_GATEWAY_PORT",
  "ALPHACLAW_NIX_MODE",
];
...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("alphaclaw gateway install", env)}`);
    if (fileLog) {
  ...
  if (runtime.status === "stopped") {
    ...
    if (process.platform === "darwin") {
      ...
      const task = resolveGatewayWindowsTaskName(env.ALPHACLAW_PROFILE);
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("alphaclaw gateway install", env),
    formatCliCommand("alphaclaw gateway", env),
  ];
