...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("alphaclaw node install"), formatCliCommand("alphaclaw node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("alphaclaw node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    ALPHACLAW_LOG_PREFIX: baseEnv.ALPHACLAW_LOG_PREFIX ?? "node",
