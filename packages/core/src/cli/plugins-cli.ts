...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: AlphaClawConfig; warnings: string[] } {
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage AlphaClaw plugins/extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.alphaclaw.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.alphaclaw.ai/plugin");
      lines.push("");
