...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.alphaclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.ALPHACLAW_PROFILE = profile;

  const stateDir = env.ALPHACLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.ALPHACLAW_STATE_DIR?.trim()) {
    env.ALPHACLAW_STATE_DIR = stateDir;
  }

  if (!env.ALPHACLAW_CONFIG_PATH?.trim()) {
    env.ALPHACLAW_CONFIG_PATH = path.join(stateDir, "alphaclaw.json");
  }

  if (profile === "dev" && !env.ALPHACLAW_GATEWAY_PORT?.trim()) {
    env.ALPHACLAW_GATEWAY_PORT = "19001";
  }
}
