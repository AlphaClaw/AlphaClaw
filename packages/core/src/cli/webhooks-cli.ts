...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.alphaclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + AlphaClaw hooks")
    ...
    .option("--hook-url <url>", "AlphaClaw hook URL")
    .option("--hook-token <token>", "AlphaClaw hook token")
