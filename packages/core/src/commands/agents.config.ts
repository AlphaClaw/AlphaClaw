...
export function listAgentEntries(cfg: AlphaClawConfig): AgentEntry[] {
...
function resolveAgentName(cfg: AlphaClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: AlphaClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AlphaClawConfig): AgentSummary[] {
