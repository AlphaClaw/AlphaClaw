...
import type { AlphaClawConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: AlphaClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("alphaclaw security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("alphaclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("alphaclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("alphaclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("alphaclaw config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
