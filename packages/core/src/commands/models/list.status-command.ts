...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAlphaClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("alphaclaw models auth setup-token")}\` or \`${formatCliCommand("alphaclaw configure")}\`.`
          : `Run \`${formatCliCommand("alphaclaw configure")}\` or set an API key env var.`;
