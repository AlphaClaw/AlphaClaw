...
export async function statusCommand(
  ...
  runtime.log(theme.heading("AlphaClaw status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("alphaclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("alphaclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.alphaclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.alphaclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("alphaclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("alphaclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("alphaclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("alphaclaw gateway probe")}`);
  }
}
