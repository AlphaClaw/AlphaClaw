...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.ALPHACLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.ALPHACLAW_STATE_DIR;
  const configPath = env.ALPHACLAW_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    ALPHACLAW_PROFILE: profile,
    ALPHACLAW_STATE_DIR: stateDir,
    ALPHACLAW_CONFIG_PATH: configPath,
    ALPHACLAW_GATEWAY_PORT: String(port),
    ALPHACLAW_GATEWAY_TOKEN: token,
    ALPHACLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    ALPHACLAW_SYSTEMD_UNIT: systemdUnit,
    ALPHACLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    ALPHACLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    ALPHACLAW_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    ALPHACLAW_STATE_DIR: stateDir,
    ALPHACLAW_CONFIG_PATH: configPath,
    ALPHACLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    ALPHACLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    ALPHACLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    ALPHACLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    ALPHACLAW_LOG_PREFIX: "node",
    ALPHACLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    ALPHACLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    ALPHACLAW_SERVICE_VERSION: VERSION,
  };
}
