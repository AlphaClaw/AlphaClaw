...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.ALPHACLAW_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.ALPHACLAW_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.ALPHACLAW_NODE_OPTIONS_READY = "1";
  ...
  child.once("error", (error) => {
    console.error(
      "[alphaclaw] Failed to respawn CLI:",
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[alphaclaw] ${parsed.error}`);
    process.exit(2);
  }
