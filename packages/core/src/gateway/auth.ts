...
export function resolveGatewayAuth(params: {
  ...
  const token =
    authConfig.token ?? env.ALPHACLAW_GATEWAY_TOKEN ?? env.ALPHACLAW_GATEWAY_TOKEN ?? undefined;
  const password =
    authConfig.password ??
    env.ALPHACLAW_GATEWAY_PASSWORD ??
    env.ALPHACLAW_GATEWAY_PASSWORD ??
    undefined;
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or ALPHACLAW_GATEWAY_TOKEN)",
    );
  }
