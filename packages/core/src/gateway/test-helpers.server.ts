...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.ALPHACLAW_STATE_DIR;
  previousConfigPath = process.env.ALPHACLAW_CONFIG_PATH;
  previousSkipBrowserControl = process.env.ALPHACLAW_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.ALPHACLAW_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.ALPHACLAW_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "alphaclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.ALPHACLAW_STATE_DIR = path.join(tempHome, ".alphaclaw");
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "alphaclaw-test-"))
    : path.join(tempHome, ".alphaclaw-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.ALPHACLAW_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.ALPHACLAW_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.ALPHACLAW_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.ALPHACLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ALPHACLAW_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.ALPHACLAW_SKIP_CANVAS_HOST;
    } else {
      process.env.ALPHACLAW_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.ALPHACLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ALPHACLAW_GATEWAY_TOKEN = fallbackToken;
  }
