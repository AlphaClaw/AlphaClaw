...
homepage: https://docs.alphaclaw.ai/hooks#command-logger
metadata:
  {
    "alphaclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.alphaclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.alphaclaw/logs/commands.log ~/.alphaclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/alphaclaw`:
   ```
   /home/username/.alphaclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
