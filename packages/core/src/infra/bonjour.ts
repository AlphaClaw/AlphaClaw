...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ALPHACLAW_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "AlphaClaw";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(AlphaClaw\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.ALPHACLAW_MDNS_HOSTNAME?.trim() ||
    process.env.ALPHACLAW_MDNS_HOSTNAME?.trim() ||
    "alphaclaw";
  const hostname =
    hostnameRaw
