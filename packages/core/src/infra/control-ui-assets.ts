...
import { resolveAlphaClawPackageRoot, resolveAlphaClawPackageRootSync } from "./alphaclaw-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveAlphaClawPackageRoot({ argv1: normalized });
  if (!packageRoot) {
    return null;
  }
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveAlphaClawPackageRootSync({
    argv1,
  ...
  if (argv1Dir) {
    // alphaclaw.mjs or dist/<bundle>.js
