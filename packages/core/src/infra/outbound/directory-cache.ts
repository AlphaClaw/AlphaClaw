...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AlphaClawConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: AlphaClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AlphaClawConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: AlphaClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: AlphaClawConfig): void {
