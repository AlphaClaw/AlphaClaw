...
export function triggerAlphaClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.ALPHACLAW_SYSTEMD_UNIT,
        process.env.ALPHACLAW_PROFILE,
      );
  ...
  const label =
    process.env.ALPHACLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ALPHACLAW_PROFILE);
