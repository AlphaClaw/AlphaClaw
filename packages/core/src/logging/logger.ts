...
export const DEFAULT_LOG_DIR = "/tmp/alphaclaw";
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "alphaclaw.log"); // legacy single-file path

const LOG_PREFIX = "alphaclaw";
...
function resolveSettings(): ResolvedSettings {
  let cfg: AlphaClawConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => AlphaClawConfig;
      };
