...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `alphaclaw --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.ALPHACLAW_GATEWAY_PORT ??
    process.env.ALPHACLAW_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.ALPHACLAW_GATEWAY_BIND ??
    process.env.ALPHACLAW_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
...
void main().catch((err) => {
  console.error(
    "[alphaclaw] Gateway daemon failed:",
