...
`alphaclaw.invoke --each`) without adding new AlphaClaw code per workflow.
...
This extension depends on AlphaClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** AlphaClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.alphaclaw/extensions` as a standalone plugin directory.
