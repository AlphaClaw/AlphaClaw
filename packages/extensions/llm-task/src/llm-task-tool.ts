...
// NOTE: This extension is intended to be bundled with AlphaClaw.
// When running from source (tests/dev), AlphaClaw internals live under src/.
...
export function createLlmTaskTool(api: AlphaClawPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via alphaclaw.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alphaclaw-llm-task-"));
