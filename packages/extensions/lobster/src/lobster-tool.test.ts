...
import type { AlphaClawPluginApi, AlphaClawPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "alphaclaw-lobster-plugin-") {
...
function fakeApi(overrides: Partial<AlphaClawPluginApi> = {}): AlphaClawPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<AlphaClawPluginToolContext> = {}): AlphaClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("tolerates noisy stdout before the JSON envelope", async () => {
    ...
    const { dir } = await writeFakeLobsterScript(
      `const payload = ${JSON.stringify(payload)};\n` +
        ...
        `process.stdout.write(JSON.stringify(payload));\n`,
      "alphaclaw-lobster-plugin-noisy-",
    );
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: AlphaClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
