...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["alphaclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["alphaclaw", "pollId"]) ??
    readNestedString(value, ["alphaclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "alphaclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "alphaclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          alphaclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "alphaclaw poll vote",
          displayText: "Vote recorded",
          value: { alphaclawPollId: pollId, pollId },
        },
      },
    ],
  };
