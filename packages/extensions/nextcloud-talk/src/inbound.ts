import {
  ...
  type AlphaClawConfig,
  type RuntimeEnv,
} from "alphaclaw/plugin-sdk";
...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as AlphaClawConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as AlphaClawConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as AlphaClawConfig);
