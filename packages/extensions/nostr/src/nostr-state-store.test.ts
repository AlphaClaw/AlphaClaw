...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.ALPHACLAW_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "alphaclaw-nostr-"));
  process.env.ALPHACLAW_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.ALPHACLAW_STATE_DIR?.trim() || env.ALPHACLAW_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".alphaclaw");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.ALPHACLAW_STATE_DIR = previous;
    }
