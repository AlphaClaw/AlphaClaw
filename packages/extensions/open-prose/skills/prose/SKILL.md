...
metadata: { "alphaclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == AlphaClaw `sessions_spawn`
- **File I/O** == AlphaClaw `read`/`write`
- **Remote fetch** == AlphaClaw `web_fetch` (or `exec` with curl when POST is required)
