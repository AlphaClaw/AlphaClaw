...
import type { ReplyPayload, AlphaClawConfig } from "alphaclaw/plugin-sdk";
import { createReplyPrefixOptions } from "alphaclaw/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // AlphaClawConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as AlphaClawConfig;
