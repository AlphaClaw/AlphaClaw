/**
 * Twitch channel plugin for AlphaClaw.
 *
...
import { buildChannelConfigSchema } from "alphaclaw/plugin-sdk";
import type {
 ...
 * for AlphaClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: AlphaClawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: AlphaClawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: AlphaClawConfig): boolean => {
