...
function resolveAlphaClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.ALPHACLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set ALPHACLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveAlphaClawRoot(), "dist", "extensionAPI.js");
