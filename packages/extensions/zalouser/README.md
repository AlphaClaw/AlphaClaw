...
AlphaClaw extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
alphaclaw channels login --channel zalouser
...
```bash
alphaclaw message send --channel zalouser --target <threadId> --message "Hello from AlphaClaw!"
```
...
```bash
alphaclaw channels login --channel zalouser              # Login via QR
alphaclaw channels login --channel zalouser --account work
alphaclaw channels status --probe
alphaclaw channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
alphaclaw directory self --channel zalouser
alphaclaw directory peers list --channel zalouser --query "name"
alphaclaw directory groups list --channel zalouser --query "work"
alphaclaw directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
alphaclaw message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
alphaclaw message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `alphaclaw channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
alphaclaw channels login --channel zalouser --account work
alphaclaw message send --channel zalouser --account work --target <id> --message "Hello"
