...
import { parseAgentSessionKey } from "../../../AlphaClaw/src/routing/session-key.js";
import { ChatHost, refreshChatAvatar } from "./app-chat.ts";
import { renderChatControls, renderTab, renderThemeToggle } from "./app-render.helpers.ts";
import { AlphaClawApp } from "./app.ts";
        ...
        <div class="topbar-left">
          ...
          <div class="brand">
            <div class="brand-logo">
              <img src="${logoHref}" alt="AlphaClaw" />
            </div>
            <div class="brand-text">
              <div class="brand-title">ALPHACLAW</div>
          ...
          state.tab === "overview"
            ? renderOverview({
                ...
                onSessionKeyChange: (next) => {
                  ...
                  state.chatMessage = "";
                  (state as unknown as AlphaClawApp).resetToolStream();
                  state.applySettings({
                    ...state.settings,
                    sessionKey: next,
          ...
          state.tab === "chat"
            ? renderChat({
                ...
                onSessionKeyChange: (next) => {
                  ...
                  (state as unknown as AlphaClawApp).chatStreamStartedAt = null;
                  state.chatQueue = [];
                  (state as unknown as AlphaClawApp).resetToolStream();
                  (state as unknown as AlphaClawApp).resetChatScroll();
                  state.applySettings({
                    ...state.settings,
                    sessionKey: next,
                    lastActiveSessionKey: next,
                ...
                onChatScroll: (event) => (state as unknown as AlphaClawApp).handleChatScroll(event),
                onDraftChange: (next) => (state.chatMessage = next),
                attachments: state.chatAttachments,
                onAttachmentsChange: (next) => (state.chatAttachments = next),
                onSend: () => (state as unknown as AlphaClawApp).handleSendChat(),
                canAbort: Boolean(state.chatRunId),
                onAbort: () => void (state as unknown as AlphaClawApp).handleAbortChat(),
                onQueueRemove: (id) => (state as unknown as AlphaClawApp).removeQueuedMessage(id),
                onNewSession: () =>
                  (state as unknown as AlphaClawApp).handleSendChat("/new", { restoreDraft: true }),
                showNewMessages: state.chatNewMessagesBelow,
                onScrollToBottom: () => state.scrollToBottom(),
                // Sidebar props for tool output viewing
                sidebarOpen: (state as unknown as AlphaClawApp).sidebarOpen,
                sidebarContent: (state as unknown as AlphaClawApp).sidebarContent,
                sidebarError: (state as unknown as AlphaClawApp).sidebarError,
                splitRatio: (state as unknown as AlphaClawApp).splitRatio,
                onOpenSidebar: (content: string) =>
                  (state as unknown as AlphaClawApp).handleOpenSidebar(content),
                onCloseSidebar: () => (state as unknown as AlphaClawApp).handleCloseSidebar(),
                onSplitRatioChange: (ratio: number) =>
                  (state as unknown as AlphaClawApp).handleSplitRatioChange(ratio),
        ...
        ${
          state.tab === "config"
            ? renderConfig({
                ...
                searchQuery: (state as unknown as AlphaClawApp).configSearchQuery,
                activeSection: (state as unknown as AlphaClawApp).configActiveSection,
                activeSubsection: (state as unknown as AlphaClawApp).configActiveSubsection,
                onRawChange: (next) => {
                  state.configRaw = next;
                },
                onFormModeChange: (mode) => (state.configFormMode = mode),
                onFormPatch: (path, value) =>
                  updateConfigFormValue(state as unknown as AlphaClawApp, path, value),
                onSearchChange: (query) =>
                  ((state as unknown as AlphaClawApp).configSearchQuery = query),
                onSectionChange: (section) => {
                  (state as unknown as AlphaClawApp).configActiveSection = section;
                  (state as unknown as AlphaClawApp).configActiveSubsection = null;
                },
                onSubsectionChange: (section) =>
                  ((state as unknown as AlphaClawApp).configActiveSubsection = section),
                onReload: () => loadConfig(state as unknown as AlphaClawApp),
                onSave: () => saveConfig(state as unknown as AlphaClawApp),
                onApply: () => applyConfig(state as unknown as AlphaClawApp),
                onUpdate: () => runUpdate(state as unknown as AlphaClawApp),
              })
            : nothing
        }
          ...
          state.tab === "logs"
            ? renderLogs({
                ...
                onExport: (lines, label) =>
                  (state as unknown as AlphaClawApp).exportLogs(lines, label),
                onScroll: (event) => (state as unknown as AlphaClawApp).handleLogsScroll(event),
              })
            : nothing
        }
      </main>
