import type { AlphaClawApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as AlphaClawApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as AlphaClawApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as AlphaClawApp);
  }
  if (host.tab === "agents") {
    const app = host as unknown as AlphaClawApp;
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as AlphaClawApp);
    await loadDevices(host as unknown as AlphaClawApp);
    await loadConfig(host as unknown as AlphaClawApp);
    await loadExecApprovals(host as unknown as AlphaClawApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as AlphaClawApp);
    await loadConfig(host as unknown as AlphaClawApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as AlphaClawApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as AlphaClawApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__ALPHACLAW_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as AlphaClawApp, false),
    loadPresence(host as unknown as AlphaClawApp),
    loadSessions(host as unknown as AlphaClawApp),
    loadCronStatus(host as unknown as AlphaClawApp),
    loadDebug(host as unknown as AlphaClawApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as AlphaClawApp, true),
    loadConfigSchema(host as unknown as AlphaClawApp),
    loadConfig(host as unknown as AlphaClawApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as AlphaClawApp, false),
    loadCronStatus(host as unknown as AlphaClawApp),
    loadCronJobs(host as unknown as AlphaClawApp),
  ]);
}
