...
const originalConnect = AlphaClawApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("alphaclaw-app") as AlphaClawApp;
...
beforeEach(() => {
  AlphaClawApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__ALPHACLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  AlphaClawApp.prototype.connect = originalConnect;
  window.__ALPHACLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/alphaclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/alphaclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/alphaclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ALPHACLAW_CONTROL_UI_BASE_PATH__ = "/alphaclaw";
    const app = mountApp("/alphaclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/alphaclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/alphaclaw/sessions");
  });
